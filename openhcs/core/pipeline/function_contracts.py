...
def special_outputs(*output_specs) -> Callable[[F], F]:
    ...
    Args:
        *output_specs: Either strings or (string, MaterializationSpec) tuples
                      ...
                      - Tuple: ("cell_counts", MaterializationSpec(TabularOptions(...))) - with materialization spec
                        Handler name is automatically inferred from options type
    ...
    Examples:
        ...
        @special_outputs(("cell_counts", MaterializationSpec(TabularOptions(...))))  # With materialization spec
        def count_cells(image):
            return processed_image, cell_count_results

        @special_outputs("positions", ("cell_counts", MaterializationSpec(TabularOptions(...))))  # Mixed
