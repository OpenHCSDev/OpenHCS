...
# ============================================================================
# PERFORMANCE OPTIMIZATION: Pre-cache config type analysis at import time
# ============================================================================

def _precache_config_analysis():
    """
    Pre-cache parameter analysis for all config types at module import time.

    Uses the @global_pipeline_config decorator registry to automatically discover
    all config types, eliminating manual list maintenance.

    This eliminates the 1+ second delay on first form load by doing the analysis
    once at import time instead of lazily on first use. Subsequent form loads
    will hit the cache and be 6-10x faster.

    Impact: First load 1300ms → 200ms (same as second load)
    """
    try:
        from openhcs.ui.shared.unified_parameter_analyzer import UnifiedParameterAnalyzer
        from openhcs.config_framework.lazy_factory import _pending_injections

        # Collect all config types from the decorator registry
        config_types = [GlobalPipelineConfig]  # Start with root config

        # Extract all decorated config types from the registry
        for target_name, injection_data in _pending_injections.items():
            for config_info in injection_data.get('configs_to_inject', []):
                config_class = config_info.get('config_class')
                if config_class is not None:
                    config_types.append(config_class)

        # Pre-cache analysis for each type
        cached_count = 0
        for config_type in config_types:
            try:
                ...
                # Ignore errors during pre-caching - don't break import
                logger.debug(f"Failed to pre-cache {config_type.__name__}: {e}")

        logger.debug(f"Pre-cached parameter analysis for {cached_count}/{len(config_types)} config types")

    except ImportError:
        # UI modules not available (e.g., running tests without GUI dependencies)
        pass

# Call at module level - runs once when config.py is imported
_precache_config_analysis()
