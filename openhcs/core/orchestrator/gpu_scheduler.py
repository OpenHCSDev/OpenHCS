...
# Async GPU initialization support
_gpu_init_thread = None
...
def setup_global_gpu_registry_fast() -> Dict[str, bool]:
    ...
    Returns dict of installed libraries. Does not populate GPU registry.
    """
    installed = check_installed_gpu_libraries()
    logger.info(f"GPU libraries installed: {installed}")
    return installed
...
def setup_global_gpu_registry_lazy():
    ...
    GPU registry will be populated on first access.
    """
    logger.info("GPU registry initialization deferred (lazy mode)")


def setup_global_gpu_registry_async(global_config=None):
    """
    Async GPU registry setup - detect in background thread.

    Returns immediately. Use wait_for_gpu_registry() to block until complete.
    """
    global _gpu_init_thread
    if _gpu_init_thread is None:
        _gpu_init_thread = threading.Thread(
            target=_init_gpu_registry_background,
            args=(global_config,),
            daemon=True
        )
        _gpu_init_thread.start()
        logger.info("GPU registry initialization started in background")


def _init_gpu_registry_background(global_config):
    """Background thread target for async GPU initialization."""
    try:
        setup_global_gpu_registry(global_config)
    finally:
        _gpu_init_complete.set()


def wait_for_gpu_registry(timeout: float = 30.0) -> bool:
    """
    Wait for async GPU registry initialization to complete.
    ...
    """
    return _gpu_init_complete.wait(timeout)
