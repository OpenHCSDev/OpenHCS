...
class OpenHCSExecutionServer:
    ...
    def _handle_execute(self, msg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "plate_id" not in msg or "pipeline_code" not in msg:
            return {
                "status": "error",
                "message": "Missing required fields: plate_id, pipeline_code",
            }

        if "config_params" not in msg and "config_code" not in msg:
            return {
                "status": "error",
                "message": "Missing config: provide either config_params or config_code",
            }
        ...
        try:
            results = self._execute_pipeline(
                ...
                ),  # May be None - separate PipelineConfig code
            ...
            return {"status": "error", "execution_id": execution_id, "message": str(e)}
    ...
    def _handle_status(self, msg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_executions": sum(
                1 for r in self.active_executions.values() if r["status"] == "running"
            ),
    ...
    def _execute_pipeline(
        ...
        try:
            ...
            if config_code:
                ...
                if pipeline_config_code:
                    ...
                    if not pipeline_config:
                        raise ValueError(
                            "pipeline_config_code must define 'config' variable"
                        )
                else:
                    # Use defaults
                ...
                if omero_config:
                    ...
                    config_params["materialization_backend"] = (
                        "zarr"  # Force zarr for OMERO
                    )

                global_config = GlobalPipelineConfig(
                    num_workers=config_params.get("num_workers", 4),
                    path_planning_config=PathPlanningConfig(
                        output_dir_suffix=config_params.get(
            ...
            if client_address:
                pipeline_steps = self._update_streaming_configs(
                    pipeline_steps, client_address
                )
    ...
    def _update_streaming_configs(self, pipeline_steps, client_address):
        ...
        for step in pipeline_steps:
            if (
                hasattr(step, "napari_streaming_config")
                and step.napari_streaming_config
            ):
                new_config = replace(
                    step.napari_streaming_config, host=client_host, port=client_port
                )
