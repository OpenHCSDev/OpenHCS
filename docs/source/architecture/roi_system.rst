...
- Generic ROI extraction from labeled segmentation masks
- Immutable ROI dataclasses with metadata (area, perimeter, centroid, bbox)
- Multi-backend materialization (disk, OMERO, Napari, Fiji)
- Per-channel materialization for dict patterns
- Real-time streaming to visualization backends
