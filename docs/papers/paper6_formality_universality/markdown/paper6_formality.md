# Paper 6: Formality as Universality

**Status**: Draft | **Target**: TOPLAS | **Lean**: TBD lines, 0 sorry

A type theory of argument epistemology. Proves formal ↔ universal equivalence, formalizes "opinion" as a complexity class, and characterizes why "in your model" objections without witnesses constitute cheap talk.

---

# Abstract

We prove that formality and universality are equivalent properties of propositions: a claim is formally verifiable if and only if it is universally accessible across all agents, cultures, times, and interpretive frameworks. This equivalence explains why machine-checked proofs "compile everywhere" while informal arguments require shared context to convey meaning.

We formalize "opinion" as a complexity class—propositions where truth exists but model-finding is intractable—and show that as sufficient coordinates become identifiable, opinion evaporates into fact.

We characterize "in your model" objections against formally proven theorems. Each such objection instantiates one of four *universe denial forms*: denying the axis type, denying the domain type, denying the proof, or denying logic itself. We prove no witness exists for any form when theorems quantify universally over open structures and compile without gaps. Therefore, "in your model" objections without witnesses constitute cheap talk (Paper 5): bounded credibility, fails to parse as argument.

Finally, we define coherent discourse agents and show that incoherent positions reflect computational limitations rather than moral failures. The appropriate response to incoherence is the type error—not contempt.
...
theorem formal_iff_universal :
  formal P ↔ universal P
```
...
**Corollary**: The informal is local by definition. It requires shared context, shared assumptions, shared language, shared culture. Remove any of these, usefulness collapses. Therefore not universal.
...
## 1.2 Opinion as Complexity Class
...
But sometimes the model is small. Sometimes the sufficient coordinates are few. Sometimes it compiles in seconds. Then "opinion" evaporates. There's just the answer.
...
## 1.3 Connection to the Pentalogy

- **Paper 1**: Typing discipline—nominal dominates structural (proven formally)
- **Paper 4**: Decision quotient—which coordinates to model (complexity)
- **Paper 5**: Credibility theory—cheap talk bounds, costly signals
- **Paper 6 (this paper)**: Why formal proofs are universal, why objections without witnesses are cheap talk

---

# 2. Universe Denial Forms

When a theorem quantifies universally over types:
...
The quantifiers are `∀ A : AxisSet` and `∀ D : Domain`. Not "for all A in model M." For all A. Period.

"In your model" objections must instantiate one of:
...
# 3. No Witness Theorems

But:
- **Axis** is an open structure (any carrier + lattice)
- **Domain** is `List (Query α)` (any list of queries)
- **Proof** has 0 sorry (no gaps exist)
- **∀** is standard (denying it exits mathematics)
...
# 4. Cheap Talk Connection

The "in your model" objection without witness:
...
Connecting to Paper 5:

```lean
theorem cheap_talk_bounded :
  is_cheap_talk u → credibility u ≤ cheap_talk_bound :=
  Paper5.cheap_talk_bound

theorem in_your_model_credibility_bounded :
  ∀ obj : InYourModelObjection,
  obj.witness = none →
  credibility obj.uttered ≤ cheap_talk_bound := by
  intro obj h_none
  have h_cheap := in_your_model_without_witness_is_cheap_talk obj h_none
  exact cheap_talk_bounded h_cheap
```

Against a costly signal (compiled proof):
...
**The asymmetry is total.** Compiled proof: credibility 1. Witnessless objection: credibility bounded by cheap talk. The objection isn't wrong. It's *nothing*. The type is empty.
...
Not claiming infallibility. Claiming accountability to the checker.

---

# 6. Incoherence as Computational Limitation
...
The correct response to incoherence is the **type error**. Not contempt. Just:

> "This doesn't compile. Here's why. Revise and resubmit."
...
The position itself is incoherent:

```lean
theorem universe_denial_position_incoherent :
  ∀ obj : InYourModelObjection,
  obj.target.isSome →      -- claims specific denial
  obj.witness = none →     -- but no witness
  (universe_denial_incoherent obj.target.get!) →  -- and no witness exists
  Incoherent obj := by
  intro obj h_target h_none h_impossible
  -- Claims ∃ witness for a form where ¬∃ witness
  -- This is claiming to inhabit empty type
  exact claim_of_impossible_witness_incoherent h_target h_none h_impossible
```
...
The structure of valid objection:
1. Identify which denial form
2. Provide witness
3. Submit to verification

Without (2), the utterance fails to parse. It's not an argument. It's noise.

## 7.2 For AI Systems

Machine-checked proofs are the ultimate costly signal. The cost is:
- Correct formalization (hard)
- Compilation (falsifiable)
- Zero sorry (verifiable)

This cost is truth-dependent: you can't fake a compiling proof of a false theorem.

## 7.3 For Epistemology

Formality is not a preference. It's the necessary condition for universality. The choice isn't "formal vs informal"—it's "universal vs local."

If you want your claims to cross boundaries (cultural, temporal, linguistic), you must formalize. There is no other path.
...
1. **Formal ↔ Universal**: Machine-checked proofs compile everywhere because formality eliminates interpretation.
...
6. **Incoherence ≠ Evil**: Computational limitation. Response: type error, not contempt.
...
TBD - Proof development in progress.
