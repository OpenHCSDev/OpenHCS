...
\begin{itemize}
\item \textbf{The necessity of nominal tags is a theorem, not a preference.} In information-barrier domains (Definition~\ref{def:info-barrier-domain}), any scheme achieving zero-error identification ($D = 0$) requires tag length $L \ge \log_2 k$ (Theorem~\ref{thm:converse}). Nominal-tag observation achieves the unique Pareto-optimal $D=0$ point with $W = O(1)$ at this bound (Theorem~\ref{thm:lwd-optimal}).
...
Classification under observational constraints admits a clean information-theoretic analysis. In information-barrier domains (Definition~\ref{def:info-barrier-domain}), nominal-tag observation achieves the unique Pareto-optimal $D=0$ point in the $(L, W, D)$ tradeoff (Theorem~\ref{thm:lwd-optimal}), and any $D=0$ scheme necessarily has $L \ge \log_2 k$ (Theorem~\ref{thm:converse}). The results are universal within the stated observation model, and all proofs are machine-verified in Lean 4.
