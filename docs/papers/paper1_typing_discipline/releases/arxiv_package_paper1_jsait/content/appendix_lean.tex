...
\item
  ...
  vs O(k) vs $\Omega(m)$ complexity separation (where $m$ is the number of query sites). Proves that nominal error
  localization is O(1), interface-only (declared) is O(k), interface-only is $\Omega(m)$, and the gap
  grows without bound.
\end{enumerate}
...
\subsection{Type Universe, Normalization Registry, and
Lineage}\label{type-universe-and-registry}

We represent nominal types as atoms (\texttt{Typ := Nat}). The model uses two distinct structures:
(i) a lineage order \texttt{mro : MRO} (most-specific-first) capturing multi-step inheritance (e.g., the result of C3 linearization), and
(ii) an OpenHCS-specific normalization registry \texttt{R : Registry} that optionally rewrites certain nominal types to a canonical base type used for resolution and provenance reporting.
Importantly, \texttt{R} is not the inheritance relation; inheritance is carried by \texttt{mro}. The registry invariant \texttt{R.wellFormed} states that normalization targets are fixed points (canonical bases do not normalize further), yielding normalization idempotence.
...
\noindent Here, \texttt{mro : MRO} denotes the multi-step inheritance linearization (most-specific-first); our mechanized results assume it is a valid linearization (e.g., produced by C3 when it succeeds).
...
\item
  \textbf{Normalization-registry invariant}: \texttt{Registry.wellFormed} is assumed as an OpenHCS invariant: canonical base types are fixed points of normalization (normalization does not chain). We prove all results conditional on this invariant, but do not derive it from more primitive foundations.
\item
