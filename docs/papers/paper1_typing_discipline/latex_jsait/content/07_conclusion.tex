...
\textbf{Remark (PL instantiation).} The theorems instantiate in programming language runtimes: CPython and Java use nominal-tag observers; Python \texttt{hasattr} uses interface-only observers; TypeScript uses interface-only (declared) observers ($D = 0$, $W = O(n)$).
\end{remark}
