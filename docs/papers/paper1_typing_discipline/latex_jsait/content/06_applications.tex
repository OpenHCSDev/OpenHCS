...
\textbf{Evidence:} The CPython object model stores a pointer to the type object (\texttt{ob\_type}) in every heap-allocated value \cite{CPythonDocs}. The \texttt{type()} builtin \cite{PythonDocs} is a single pointer dereference, hence $O(1)$ time and $O(1)$ AST size.
