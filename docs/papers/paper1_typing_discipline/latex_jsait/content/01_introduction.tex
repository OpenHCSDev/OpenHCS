...
\item \textbf{Matroid structure (query complexity).} Minimal sufficient query sets form the bases of a matroid. The \emph{identification dimension} (the common cardinality of all minimal sets) lower-bounds the query cost $W$ for any tag-free scheme.
\end{enumerate}
...
\textbf{Rate-distortion theory.} The $(L, W, D)$ framework connects to Shannon's rate-distortion theory \cite{shannon1959coding, berger1971rate} with an important twist: the ``distortion'' $D$ is semantic (class misidentification), and there is a second resource $W$ (query cost) alongside rate $L$. Classical rate-distortion asks: what is the minimum rate to achieve distortion $D$? We ask: given rate $L$, what is the minimum query cost $W$ to achieve distortion $D = 0$? The Pareto frontier (Theorem~\ref{thm:lwd-optimal}) characterizes this three-dimensional tradeoff.
