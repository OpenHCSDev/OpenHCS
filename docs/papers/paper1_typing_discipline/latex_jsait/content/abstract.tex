...
\textbf{Rate-identification tradeoff.} We identify the unique Pareto-optimal zero-error point in the $(L, W, D)$ tradeoff space (under the condition that attributes are not class-injective) and describe the induced tradeoff geometry. A nominal tag of $L = \lceil \log_2 k \rceil$ bits for $k$ classes yields $W = O(1)$ with $D = 0$. Without tags ($L = 0$), identification requires $W = \Omega(n)$ queries and, in the presence of attribute collisions, incurs $D > 0$.

\textbf{Converse.} If the attribute profile is not class-injective, any scheme achieving $D = 0$ requires $L \geq \log_2 k$ bits. This is tight: nominal tagging achieves the bound with $W = O(1)$.
