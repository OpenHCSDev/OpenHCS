...
Let $E$ be the set of all type axes, and let $\mathcal{B}$ be the family of minimal complete axis sets. Then $\mathcal{B}$ satisfies the basis-exchange axiom:

For any $B_1, B_2 \in \mathcal{B}$ and any $x \in B_1 \setminus B_2$, there exists $y \in B_2 \setminus B_1$ such that $(B_1 \setminus \{x\}) \cup \{y\} \in \mathcal{B}$.

Therefore $\mathcal{B}$ is the set of bases of a matroid on $E$. By the equicardinality theorem for matroid bases, all minimal complete axis sets have equal cardinality.
\end{theorem}

\begin{proof}
See Lean formalization: \texttt{proofs/abstract\_class\_system.lean}. The proof verifies:
\begin{enumerate}
\item Basis-exchange: swapping one axis for another preserves minimal completeness
\item Equicardinality follows from standard matroid theory
\end{enumerate}
\end{proof}
