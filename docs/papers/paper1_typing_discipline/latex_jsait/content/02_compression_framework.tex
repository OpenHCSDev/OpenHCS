...
\end{cases}
\]
Here $\text{behavior}(v)$ denotes the observable behavior of $v$ under program execution.
\end{definition}
...
\item \textbf{No preprocessing over type universe}: The scheme cannot precompute a global lookup table indexed by all possible types. (With such preprocessing, any problem reduces to $O(1)$ lookups.)
