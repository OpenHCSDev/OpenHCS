...
Values $x, y \in \mathcal{V}$ are interface-equivalent, written $x \equiv_I y$, iff $\phi(x) = \phi(y)$ for all interface observations $\phi$.
\end{definition}

An interface-only procedure can only distinguish values that are not interface-equivalent. Therefore, any property computed by an interface-only procedure must be constant on equivalence classes.
...
The tag length $L$ is the number of bits required to store a type identifier per value.
\end{definition}
...
The distortion $D$ is the probability of semantic misclassification: $D = \Pr[\text{type}(v_1) = \text{type}(v_2) \text{ but } \text{behavior}(v_1) \neq \text{behavior}(v_2)]$.
\end{definition}
