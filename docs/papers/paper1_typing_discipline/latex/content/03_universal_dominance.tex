...
\textbf{Theorem 3.4 (Nominal Typing Pareto-Dominance).} When a
\texttt{bases} axis exists in the class system, nominal typing
Pareto-dominates all shape-based alternatives (provides strictly more
...
\(\{S\}\)). Let \(D_{\text{nominal}}\) be nominal typing (uses
\(\{B, S\}\); names are aliases).
...
\textbf{Conclusion: Shape-Based Typing Has Negative Expected Value.}
...
When \(B \neq \emptyset\):
\begin{itemize}
\item \(D_{\text{shape}}\) is Pareto-dominated by \(D_{\text{nominal}}\)
...
Therefore: \textbf{Choosing shape-based typing when inheritance exists
...
shape-based typing (structural typing, Protocol-based typing) is \textbf{incorrect}.
Duck typing is \textbf{incoherent} (Theorem 2.10d). It is not even a valid typing discipline.
Nominal typing is \textbf{correct}. This
...
\item
  \textbf{Information content of \((S)\).} A shape discipline
...
\end{align*}
Therefore $q_B(s, b_1) = q_B(s, b_2)$, contradicting $B$-dependence. $\blacksquare$
...
\begin{enumerate}
\item \textbf{Extend the observable interface.} Introduce an operation $\pi_B : S' \to B$ (or equivalent) that exposes $B$ information. This is \emph{adding an axis}---$B$ becomes observable.
...
\emph{Proof.} By Theorem 3.22g, if the $S$-interface is preserved, $B$-dependent queries are unanswerable. To answer them, the interface must be extended. Adding $\pi_B$ makes $B$ observable---this is precisely what ``adding an axis'' means: making new information available to queries. $\blacksquare$
...
\emph{Proof.} By independence, $\exists q$ that $\mathcal{A}$ cannot answer but $X$ can. Adding $X$ enables $q$ while preserving all existing capabilities. $\blacksquare$
...
\emph{Proof.} Any query answerable by $N$ is answerable by $f(B)$. Therefore $N$ provides no capability beyond $B$. $\blacksquare$
...
\emph{Proof.} Non-recursive values are atomic and can be stored as attribute values in the namespace. Therefore they are captured by $S$ and provide no independent capability. $\blacksquare$
