...
\item
  ...
  (Theorem 2.10d).
\item
  \textbf{Complexity separation}: Nominal typing achieves O(1) error
  localization versus duck typing's \(\Omega\)(n) (Theorem 4.3). The gap is unbounded.
\item
  \textbf{Provenance impossibility}: Duck typing cannot provide provenance
...
\textbf{The decision procedure:} Given domain requirements, the required axes are \emph{computable}. The typing discipline follows deterministically.
...
\textbf{PR \#44:} The migration from duck typing to nominal contracts is documented in a publicly verifiable pull request~\cite{openhcsPR44}: \url{https://github.com/trissim/openhcs/pull/44}. This PR eliminated 47 scattered \texttt{hasattr()} checks by introducing ABC contracts.

\textbf{Lean 4 Proofs:} The complete Lean 4 formalization (2613 lines, 127 theorems, 0 \texttt{sorry} placeholders)~\cite{openhcsLeanProofs} is included as supplementary material. Reviewers can verify the proofs by running \texttt{lake build} in the proof directory.
