...
OpenHCS is \emph{less wrong} than structural typing (\(A = \{S\}\)) or nominal typing (\(A = \{B, S\}\)) because it includes more axes. But ``less wrong'' is not ``correct.'' The theorems predict that as the space of domains grows, OpenHCS will encounter impossibility walls:
\[
...
Nominal (\(\{B, S\}\), coherent) & Yes & No & Always (when
\(B \neq \emptyset\)) \\
\end{longtable}
