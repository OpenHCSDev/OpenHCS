...
\paragraph{2.4.1 Axes (names as sugar)}\label{the-two-axis-model}
...
(attribute, value) pairs defining the type's interface. Names are
treated as syntactic sugar (aliases for structures already captured by
\(S\)) and do not add observable power; we elide them henceforth.
...
\label{tab:nbs-cross}
\small
\begin{tabular}{@{}p{1.7cm}p{3.0cm}p{3.1cm}p{3.4cm}p{1.3cm}@{}}
\toprule
Language & N (Name) & B (Bases) & S (Namespace) & Type system \\
\midrule
Python & \texttt{type(x).\_\_name\_\_} & \texttt{\_\_bases\_\_}; \texttt{\_\_mro\_\_} & \texttt{\_\_dict\_\_}; \texttt{dir()} & Nominal \\
Java   & \texttt{getClass().getName()} & \texttt{getSuperclass()}, \texttt{getInterfaces()} & \texttt{getDeclaredMethods()} & Nominal \\
Ruby   & \texttt{obj.class.name} & \texttt{ancestors} (ordered) & \texttt{methods}, \texttt{instance\_variables} & Nominal \\
C\#    & \texttt{GetType().Name} & \texttt{BaseType}, \texttt{GetInterfaces()} & \texttt{GetProperties()}, \texttt{GetMethods()} & Nominal \\
\bottomrule
\end{tabular}
\end{table}

All four languages provide \textbf{runtime access to both axes (N is derivable from B)}.
...
name and \([T_i]\) are type arguments. The \((B, S)\) model is
