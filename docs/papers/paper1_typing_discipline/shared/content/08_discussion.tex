...
\textbf{Empirical validation scope.} The formal results (Theorems 3.5,
...
empirical measurements (DTD reduction, error rate correlation, DOF
improvement) are drawn from OpenHCS. However, the \emph{direction} of
the empirical claims---that capability gaps translate to measurable
error reduction---follows from the formalism: if provenance is
...
\item
  \textbf{Singular (measured):} 47 \texttt{hasattr()} calls eliminated,
  14.2$\times$ DOF reduction, $r = -0.85$ error correlation.
\end{itemize}
