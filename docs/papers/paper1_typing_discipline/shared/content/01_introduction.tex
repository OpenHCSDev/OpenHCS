...
This work follows the tradition of Liskov \& Wing~\cite{liskov1994behavioral}, who formalized correctness criteria for subtyping in their foundational TOPLAS paper. Where Liskov \& Wing asked ``what makes subtyping \emph{correct}?'', we ask ``what makes typing discipline selection \emph{correct}?''

Our contribution is not recommending specific typing disciplines, but deriving what constitutes a correct choice from formal requirements. We prove the (B, S) model (Bases and Namespace) \textbf{completes} the semantic structure of class-based object-oriented systems, enabling derivation rather than preference-based selection.

\subsection{Overview}\label{overview}

This paper proves that for object-oriented systems with inheritance hierarchies, typing discipline selection is \textbf{derivable} from requirements rather than a matter of preference. All results are machine-checked in Lean 4 (2600+ lines, 127 theorems, 0 \texttt{sorry} placeholders).
...
\textbf{The pay-as-you-go principle:} Each axis increment adds capabilities without cognitive load increase until those capabilities are invoked. Duck typing uses $S$; nominal uses $(B,S)$ with the same \texttt{isinstance()} API; scoped resolution uses $(B,S,\text{Scope})$ with one optional parameter.
...
\item
  ...
  capabilities with zero tradeoffs). Structural typing is appropriate
  only when \texttt{bases\ =\ {[}{]}} universally (e.g., Go) or in
  retrofit/interop scenarios. The decision is \textbf{derived} from
...
\textbf{2. Completeness and Robustness Theorems (Section 3.11):} - \textbf{Theorem 3.32
(Model Completeness):} \((B, S)\) captures all runtime-available type
information. - \textbf{Theorem 3.34-3.35 (Capability Comparison):}
\(\mathcal{C}_{\text{duck}} \subsetneq \mathcal{C}_{\text{nom}}\). Nominal
provides all duck typing capabilities plus four additional. - \textbf{Lemma 3.37 (Axiom
...
\textbf{3. Metatheoretic foundations (Sections 2-3):} - The two-axis
model (B, S) as a universal framework for class systems (names are
syntactic sugar) - Theorem 2.15 (Axis Lattice Dominance): capability
monotonicity under axis subset ordering - Theorem 2.17 (Capability
Completeness): the capability set \(\mathcal{C}_B\) is exactly four
elements (complete) - Theorem 3.5: Nominal typing strictly dominates
shape-based typing universally (when \(B \neq \emptyset\))

\textbf{4. Machine-checked verification (Section 6):} - 2600+ lines of
Lean 4 proofs across five modules - 127 theorems/lemmas covering typing,
architecture, information theory, complexity bounds, impossibility,
lower bounds, completeness analysis, generics, exotic features, universal
...
\texttt{sorry} placeholders (all 127 theorems/lemmas complete)}
...
\texttt{@auto_create_decorator} $\rightarrow$ \texttt{@global_pipeline_config}
...
\item
  \textbf{When B $\neq$ $\emptyset$, nominal typing dominates.} Nominal typing
  provides strictly more capabilities. Adapters eliminate the retrofit exception (Theorem
  2.10j). When inheritance exists, nominal typing is the capability-maximizing choice.
\item
...
Boundary scope (pulled forward for clarity): when \(B = \emptyset\)
...
is the coherent choice. Our dominance claims apply whenever \(B \neq
\emptyset\) \emph{and} inheritance metadata is accessible; FFI or
opaque-runtime boundaries that erase \(B\) fall outside the claim.
