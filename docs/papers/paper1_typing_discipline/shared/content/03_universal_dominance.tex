...
which typing discipline is correct and is treated as syntactic sugar
hereafter.
...
\item Same mental load: \texttt{isinstance()} API identical for both
...
\textbf{Corollary 3.84 (Name Collapse).} The Name axis $N$ is derivable from Bases $B$: if a type has a name, it has $B$. Therefore the minimal model is $(B, S)$, not $(N, B, S)$.
