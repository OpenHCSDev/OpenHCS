...
This section establishes the computational complexity of determining which state coordinates are decision-relevant, under the succinct encoding of Section~\ref{sec:encoding}. We prove three main results:
...
SUFFICIENCY-CHECK is coNP-complete \cite{cook1971complexity, karp1972reducibility}. \emph{(Machine-verified in Lean 4; see \texttt{Hardness/CoNPComplete.lean}.)}
\end{theorem}
...
MINIMUM-SUFFICIENT-SET is coNP-complete. \emph{(Machine-verified.)}
\end{theorem}
...
ANCHOR-SUFFICIENCY is $\Sigma_2^P$-complete \cite{stockmeyer1976polynomial} (already for Boolean coordinate spaces). \emph{(Machine-verified.)}
\end{theorem}
