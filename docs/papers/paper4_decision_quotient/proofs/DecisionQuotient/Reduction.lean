...
def reductionUtility (φ : CNF n) : ReductionAction → ReductionState n → ℝ
...
/-- The decision problem constructed from a CNF formula -/
def reductionProblem (φ : CNF n) : DecisionProblem ReductionAction (ReductionState n) where
...
theorem opt_satisfying (φ : CNF n) (a : Assignment n) (hsat : φ.eval a = true) :
...
theorem opt_falsifying (φ : CNF n) (a : Assignment n) (hfalse : φ.eval a = false) :
...
theorem tautology_implies_sufficient (φ : CNF n) (htaut : φ.isTautology) :
...
theorem sufficient_implies_tautology (φ : CNF n)
...
theorem tautology_iff_sufficient (φ : CNF n) :
