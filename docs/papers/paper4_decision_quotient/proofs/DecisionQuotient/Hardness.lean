...
-/
theorem sufficiency_check_coNP_hard {n : ℕ} (φ : CNF n) :
...
theorem min_sufficient_set_coNP_hard {n : ℕ} (φ : CNF n) :
    (reductionProblem φ).isSufficient (∅ : Finset (Fin 1)) ↔ φ.isTautology :=
...
-/
theorem over_modeling_justified {n : ℕ} (φ : CNF n) :
    (reductionProblem φ).isSufficient (∅ : Finset (Fin 1)) ↔ φ.isTautology :=
...
theorem anchor_sufficiency_sigma2p (φ : ExistsForallCNF) :
    ExistsForallCNF.satisfiable φ ↔
      (qbfProblem (ExistsForallCNF.padUniversal φ)).anchorSufficient
        (xCoords (ExistsForallCNF.padUniversal φ).nx (ExistsForallCNF.padUniversal φ).ny) :=
  exists_forall_iff_anchor_sufficient_padded φ

theorem anchor_sufficiency_sigma2p_nonempty (φ : ExistsForallCNF) (hny : 0 < φ.ny) :
    ExistsForallCNF.satisfiable φ ↔
