...
def qbfEval (φ : ExistsForallCNF) (s : QBFState φ.nx φ.ny) : Bool :=
  φ.formula.eval (ExistsForallCNF.sumAssignment (xPart s) (yPart s))

@[simp] lemma qbfEval_mkState (φ : ExistsForallCNF) (x : AssignX φ.nx) (y : AssignY φ.ny) :
    qbfEval φ (mkState x y) =
      φ.formula.eval (ExistsForallCNF.sumAssignment x y) := by
  simp [qbfEval]
...
noncomputable def qbfUtility (φ : ExistsForallCNF) : QBFAction → QBFState φ.nx φ.ny → ℝ
  | yes, s => if qbfEval φ s then 2 else 0
  | no,  s => if yAllFalse s then 1 else 0

noncomputable def qbfProblem (φ : ExistsForallCNF) : DecisionProblem QBFAction (QBFState φ.nx φ.ny) where
...
lemma opt_yes_only (φ : ExistsForallCNF) (s : QBFState φ.nx φ.ny)
    (h : qbfEval φ s = true) :
...
lemma opt_no_only (φ : ExistsForallCNF) (s : QBFState φ.nx φ.ny)
...
lemma opt_both (φ : ExistsForallCNF) (s : QBFState φ.nx φ.ny)
    (hφ : qbfEval φ s = false) (hy : ¬ yAllFalse s) :
...
theorem exists_forall_iff_anchor_sufficient (φ : ExistsForallCNF) (hny : 0 < φ.ny) :
  ...
  · intro hsat
    ...
    have hxval : xPart s = x := by
      funext i; exact hx' i
    have hq' : φ.formula.eval (ExistsForallCNF.sumAssignment x (yPart s)) = true := by
      have := hx (yPart s)
      simpa [ExistsForallCNF.satisfiedBy] using this
    have hq : qbfEval φ s = true := by
      simpa [qbfEval, hxval] using hq'
    have hq0' : φ.formula.eval (ExistsForallCNF.sumAssignment x y0) = true := by
      have := hx y0
      simpa [ExistsForallCNF.satisfiedBy] using this
  ...
  · intro hanch
    ...
    · have hq' : φ.formula.eval (ExistsForallCNF.sumAssignment x y) = true := by
        simpa [qbfEval, s] using hq
      simpa [ExistsForallCNF.satisfiedBy] using hq'
...
theorem exists_forall_iff_anchor_sufficient_padded (φ : ExistsForallCNF) :
    ExistsForallCNF.satisfiable φ ↔
      (qbfProblem (ExistsForallCNF.padUniversal φ)).anchorSufficient
        (xCoords (ExistsForallCNF.padUniversal φ).nx (ExistsForallCNF.padUniversal φ).ny) := by
  have hny : 0 < (ExistsForallCNF.padUniversal φ).ny := by
    simp [ExistsForallCNF.padUniversal]
  have hpad := exists_forall_iff_anchor_sufficient (φ := ExistsForallCNF.padUniversal φ) hny
  constructor
  · intro hsat
    have hsat' : ExistsForallCNF.satisfiable (ExistsForallCNF.padUniversal φ) :=
      (ExistsForallCNF.satisfiable_iff_padUniversal φ).1 hsat
    exact hpad.mp hsat'
  · intro hanch
    have hsat' : ExistsForallCNF.satisfiable (ExistsForallCNF.padUniversal φ) :=
      hpad.mpr hanch
    exact (ExistsForallCNF.satisfiable_iff_padUniversal φ).2 hsat'
