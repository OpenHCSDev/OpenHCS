...
\item \textbf{Encoding-regime separation.} Under [E], SUFFICIENCY-CHECK is polynomial in $|S|$. Under [S+ETH], there exist succinct worst-case instances (with $k^*=n$) requiring $2^{\Omega(n)}$ time. Under [Q\_bool], mechanized lower bounds of $\Omega(2^n)$ hold for both `Opt`-oracle and value-entry interfaces (via the $I=\emptyset$ subproblem). Other intermediate access models remain open.
\end{enumerate}
...
\item \textbf{[Q\_bool] query-access lower bounds:} worst-case full-problem query complexity is $\Omega(2^n)$ for both `Opt`-oracle and value-entry interfaces (Propositions~\ref{prop:query-regime-obstruction}, \ref{prop:query-value-entry-lb}).
