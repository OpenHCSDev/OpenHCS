...
\item In the intermediate query-access model: the mechanized Boolean query submodel yields a full worst-case lower bound $\Omega(2^n)$ for SUFFICIENCY-CHECK via an oracle-indistinguishable yes/no pair for the $I=\emptyset$ subproblem (Proposition~\ref{prop:query-regime-obstruction}).
...
The explicit ETH lower bound is still a succinct worst-case statement; Proposition~\ref{prop:query-regime-obstruction} strengthens the intermediate-regime treatment with a theorem-level query lower bound in the mechanized Boolean oracle model.
