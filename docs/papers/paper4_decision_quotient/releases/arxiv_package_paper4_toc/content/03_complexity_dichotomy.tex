...
\paragraph{Model note.} Part~1 is [E] (time polynomial in $|S|$). Part~2 is [S+ETH] (time exponential in $n$). The encodings are defined in Section~\ref{sec:encoding}. These two parts are stated in different encodings and are not directly comparable as functions of a single input length.
