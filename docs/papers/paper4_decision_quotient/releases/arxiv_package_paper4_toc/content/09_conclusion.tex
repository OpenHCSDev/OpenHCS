...
While this work is theoretical, the complexity bounds have practical meaning. Under ETH and the succinct encoding, one should not expect generic polynomial-time worst-case solvers for arbitrary inputs. This is a diagnostic statement, not a claim of permanent impossibility in all regimes: when this barrier appears, the correct response is to refine representation and assumptions until decision relevance becomes structurally tractable, or to make approximation commitments explicit. Theorem~\ref{thm:overmodel-diagnostic} states this interpretive point directly at the level of parameterization. In practice, real datasets often have structure (sparsity, bounded dependency depth, separable utilities) that the tractable subcases capture. Thus, engineering efforts should focus on (a) detecting and exploiting structural restrictions in inputs, and (b) designing heuristic or approximate methods for currently unstructured instances while those structural hypotheses are developed.
