...
\item \textbf{[Q\_bool] query-access lower bounds:} worst-case full-problem query complexity is $\Omega(2^n)$ for `Opt`-oracle, value-entry, and state-batch interfaces (Propositions~\ref{prop:query-regime-obstruction}--\ref{prop:oracle-lattice-strict}), with randomized weighted robustness and oracle-lattice closures.
