...
\paragraph{Definitional equality for simple cases.} Where possible, we defined concepts so that simple cases reduce definitionally:
\begin{verbatim}
...
We implemented (1) as a macro; (2)--(4) remain manual. A \texttt{complexity} tactic analogous to \texttt{continuity} or \texttt{measurability} would significantly accelerate future work.
...
\paragraph{Separate ``math'' from ``encoding.''} The mathematical content (``sufficiency is the same as tautology under this encoding'') should be separate from encoding details (``how to represent formulas as coordinates''). This separation aids both clarity and reuse.
