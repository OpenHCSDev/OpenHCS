...
These results integrate with the leverage framework (Paper 3): credibility leverage $L_C = \Delta C / \text{Signal Cost}$ is maximized by minimizing cheap talk and maximizing costly signal exposure. Claims are regime-typed by channel (\texttt{[CT]} cheap-talk, \texttt{[VS]} verifier-backed) and audience domain (\texttt{[M]}, \texttt{[S]}). The theorems are formalized in Lean 4.
