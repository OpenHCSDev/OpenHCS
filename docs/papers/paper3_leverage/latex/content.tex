...
\textbf{Empirical Validation:} 13 structural witnesses from OpenHCS
(45K LoC). DOF collapses yield leverage factors $\rho$ from 5$\times$
to 120$\times$; many refactorings reduce DOF to 1.

All theorems machine-checked in Lean 4 (1,634 lines, 142 definitions/theorems, \textbf{0 sorry placeholders}).
...
\subsection{Axiom Zero}

Before formalizing leverage, we state the primitive axiom from which the framework derives:

\begin{quote}
\textbf{Axiom 0:} \emph{Mattering is the only thing that matters.}
\end{quote}

This is not a tautology. It is a \emph{selection} from possible objective functions. One could optimize for comfort, approval, safety, or process compliance. Axiom 0 asserts that impact---whether an action contributes to outcomes that matter---is the sole criterion for evaluation.
...
\textbf{6. Empirical Validation (Section 6):} 13 structural witnesses
from OpenHCS; DOF collapses yield $\rho$ factors 5$\times$--120$\times$
(many collapse to a single locus).

\textbf{7. Machine-Checked Proofs (Appendix A):} All theorems formalized in Lean 4 (1,634 lines across 7 modules, 142 definitions/theorems, \textbf{0 sorry placeholders}).
...
\textbf{Interpretation:} Each DOF is a ``component'' that must work correctly. This is the reliability analog of our independence assumption (Axiom \ref{ax:independent-errors}).
...
\begin{theorem}[Theorem 3.0: Leverage Maximization Principle]\label{thm:leverage-max}
For any architectural decision with alternatives $A_1, \ldots, A_n$, the optimal choice maximizes leverage:
\[
...
This is \textbf{THE central theorem} of this paper. All subsequent results are instances or corollaries:
\begin{itemize}
\item Theorem 4.1 (SSOT Instance): $L(\text{SSOT}) = \infty > L(\text{non-SSOT})$
\item Theorem 4.2 (Typing Instance): $L(\text{nominal}) > L(\text{duck})$
\item Theorem 4.3 (Microservices): $L$ determines optimal service count
\end{itemize}
...
\section{Empirical Validation (Structural Quantification)}\label{empirical-validation}

We validate the leverage framework by \emph{instantiating} its formal
quantities—degrees of freedom (DOF), leverage, and modification
complexity—on 13 refactorings from OpenHCS (a production 45K LoC Python
bioimage analysis platform). This section uses \emph{structural
measurement}, not statistical inference: each refactoring is a witness
that (1) DOF is computable in a real system and (2) leverage improves by
the consolidation factor actually achieved.

\subsection{Structural Measurement Protocol}

For a before/after pair $A_{\text{pre}}, A_{\text{post}}$ define the
\textbf{structural leverage factor}:
\[
\rho := \frac{\mathrm{DOF}(A_{\text{pre}})}{\mathrm{DOF}(A_{\text{post}})}.
\]
If capabilities are preserved,
$|\mathrm{Cap}(A_{\text{pre}})| = |\mathrm{Cap}(A_{\text{post}})|$, then
leverage scales exactly by $\rho$:
\[
\frac{L(A_{\text{post}})}{L(A_{\text{pre}})} = \rho.
\]
If capabilities increase (common in refactoring), $\rho$ is a
\emph{lower bound} on leverage improvement.
...
Before: $\mathrm{DOF}=n_{\text{reg}}$ (define + register). After:
$\mathrm{DOF}=1$ (define; metaclass registers). $\rho = n_{\text{reg}}$.
In OpenHCS, $n_{\text{reg}}=23 \Rightarrow \rho=23\times$; one instance
eliminated 47 scattered dispatch checks.

\subsubsection{CS2: Configuration Consolidation (Convention)}

Let $m$ be explicit parameters; $k$ overrides after defaults. Before:
$\mathrm{DOF}=m$. After: $\mathrm{DOF}=k$. $\rho = m/k$. In OpenHCS,
$m=50$, $k=5 \Rightarrow \rho=10\times$.

\subsubsection{CS3: REST API Genericization}

Let $n$ be specific endpoints; $k$ generic endpoints after refactor.
Before: $\mathrm{DOF}=n$. After: $\mathrm{DOF}=k$. $\rho = n/k$. In
OpenHCS, $n=15$, $k=3 \Rightarrow \rho=5\times$.

\subsection{CS4--CS13: Structural Witnesses}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Case Study} & \textbf{DOF Pre} & \textbf{DOF Post} & \textbf{$\rho$} & \textbf{Archetype} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Structural leverage factors ($\rho$) for 13 OpenHCS refactorings}
\label{tab:case-studies}
\end{table}

\subsection{Structural Summary}

(1) DOF is computable and yields large factors: $\rho$ ranges from
5$\times$ to 120$\times$; many refactorings collapse to a single locus.

(2) Error improvement is theorem-level, not regression: with per-DOF
error rate $p$, $\mathbb{E}[\#\text{errors}] = p \cdot \mathrm{DOF}$.
Thus $\mathbb{E}_{\text{pre}}/\mathbb{E}_{\text{post}} = \rho$ whenever
capabilities are preserved; $\rho$ is conservative when capabilities
increase.

(3) Modification complexity scales with the same factor when a change
targets the consolidated concern.

\subsection{Threats to Validity (Structural)}

\textbf{Single codebase:} All witnesses from OpenHCS; other domains
could reveal different archetype frequencies but do not affect the
correctness of each $\rho$.

\textbf{DOF operationalization:} Counting independent definition loci is
coarse but robust for registries, endpoints, overrides, duplicated
schemas.

\textbf{Capability growth:} Some refactorings increase capabilities; in
those cases $\rho$ is a lower bound on leverage gain.
...
\textbf{6. Empirical Validation:} 13 structural witnesses from OpenHCS
with DOF collapses yielding $\rho$ factors from 5$\times$ to 120$\times$
(many to a single locus).
