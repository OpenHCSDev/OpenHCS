...
/-- Theorem 4.2.9: Same DOF means same expected errors.
    The advantage of nominal is MORE CAPABILITIES, not fewer errors. -/
theorem nominal_same_dof_same_errors (p : ErrorRate) :
    let nom := TypingDiscipline.Nominal.toArchitecture
    let duck := TypingDiscipline.Duck.toArchitecture
