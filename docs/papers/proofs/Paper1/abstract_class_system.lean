...
/-
  ...
  We model types abstractly using natural numbers as identifiers.
-/
...
theorem self_mem_ancestors (B : Bases) (fuel : Nat) (T : Typ) :
    T ∈ ancestors B fuel T := by
  cases fuel with
  | zero =>
      simp [ancestors]
  | succ n =>
...
theorem exact_implies_nominal (B : Bases) (fuel : Nat) (xType T : Typ) :
    exactCompatible xType T → nominalCompatible B fuel xType T := by
  intro h
  cases h
  exact self_mem_ancestors B fuel xType

-- Nominal can hold when exact-type fails (lineage uses more than head)
theorem nominal_not_implies_exact :
    ∃ (B : Bases) (fuel : Nat) (xType T : Typ),
      nominalCompatible B fuel xType T ∧ ¬ exactCompatible xType T := by
  -- Witness: xType = 0, T = 1, with 0 inheriting from 1
  let B : Bases := fun t => if t = 0 then [1] else []
  refine ⟨B, 1, 0, 1, ?_, ?_⟩
  · -- nominal: 1 ∈ ancestors B 1 0
    simp [nominalCompatible, ancestors, B]
  · -- not exact: 0 ≠ 1
    simp [exactCompatible]
...
-- The axes of a class system (Name included for historical reference only)
inductive Axis where
  | Name       -- Label only (not an independent axis; see Part 1)
...
-- Legacy definitions (Name is not a real axis, but kept for compatibility)
def nameOnly : AxisSet := [.Name]  -- Deprecated: Name is not independent
def basesOnly : AxisSet := [.Bases]

-- Capabilities enabled by each axis
def axisCapabilities (a : Axis) : List UnifiedCapability :=
  match a with
  | .Name => []  -- Name alone provides nothing (it's just a label)
...
theorem nominal_capabilities :
    axisSetCapabilities nominalAxes =
      [UnifiedCapability.interfaceCheck, .identity, .provenance, .enumeration, .conflictResolution] := rfl
...
theorem nonS_observable_forces_outside_shape
    ...
    (ns : Namespace) (obs : Typ → β) :
    (∃ A B, shapeEq ns A B ∧ obs A ≠ obs B) →
    ∃ c : β, (fun T => decide (obs T = c)) ∉ ShapeQuerySet ns := by
  ...
  have hdep : BasesDependentQuery ns q := ⟨A, B, hshape, hqdiff⟩
  -- By our iff-lemma, this means q is outside ShapeQuerySet
  exact (outside_shape_iff_bases_dependent ns q).2 hdep
...
theorem nonS_function_not_shape_simulable
    {β : Type} [DecidableEq β]
    (ns : Namespace) (f : Typ → β)
    (h_distinguishes : ∃ A B, shapeEq ns A B ∧ f A ≠ f B) :
    ¬ShapeRespecting ns f := by
  intro h_shape
...
def NamespacePreserving (ns : Namespace) (enc : Typ → Typ) : Prop :=
  ∀ T, ns (enc T) = ns T

-- If a query is Bases-dependent, no shape-respecting observer can reproduce it
-- after any namespace-preserving encoding.
theorem no_shape_simulation (ns : Namespace) (enc : Typ → Typ)
    (h_ns : NamespacePreserving ns enc) :
    ∀ q, BasesDependentQuery ns q →
      ¬ ∃ o, ShapeRespectingSingle ns o ∧ ∀ T, o (enc T) = q T := by
  intro q h_dep
  ...
  have h_enc_shape : shapeEq ns (enc A) (enc B) := by
    dsimp [shapeEq]
    have hA := h_ns A
    have hB := h_ns B
...
theorem semantic_non_embeddability (ns : Namespace) (enc : Typ → Typ)
    (h_ns : NamespacePreserving ns enc)
    (h_same_shape : ∃ A B : Typ, A ≠ B ∧ shapeEq ns A B) :
    ∃ q, BasesDependentQuery ns q ∧
      ¬ ∃ o, ShapeRespectingSingle ns o ∧ ∀ T, o (enc T) = q T := by
  -- obtain a Bases-dependent query from the existing gap theorem
  obtain ⟨q, _hqAll, hq_not_shape⟩ := shape_strict_subset ns h_same_shape
  have hq_dep : BasesDependentQuery ns q :=
    (outside_shape_iff_bases_dependent ns q).mp hq_not_shape
  refine ⟨q, hq_dep, ?_⟩
  exact no_shape_simulation ns enc h_ns q hq_dep
...
abbrev Declared := Typ → List AttrName

-- Visible namespace = union of declared attrs across ancestors (fuel-bounded), dup-free
def visibleNS (B : Bases) (decl : Declared) (fuel : Nat) : Namespace :=
  fun T => ((ancestors B fuel T).flatMap (fun u => decl u)).eraseDups

-- Canonical provenance: first ancestor (MRO order) that declared `a`
def prov (B : Bases) (decl : Declared) (fuel : Nat) (T : Typ) (a : AttrName) :
    Option Typ :=
  (ancestors B fuel T).find? (fun u => a ∈ decl u)

-- A Bool-valued query: “is provenance of (T, a) equal to provider P?”
def provIs (B : Bases) (decl : Declared) (fuel : Nat) (a : AttrName) (P : Typ) :
    SingleQuery :=
  fun T => decide (prov B decl fuel T a = some P)

-- Concrete counterexample:
-- 0 = A declares "x"
-- 2 = B1 inherits from A, declares nothing
-- 3 = B2 has no bases, declares "x" itself
def declEx : Declared
  | 0 => ["x"]
  | 3 => ["x"]
  | _ => []
...
def nsEx : Namespace := visibleNS basesEx declEx 2

-- Same visible namespace: both “have x”
example : shapeEq nsEx 2 3 := by
  unfold nsEx visibleNS shapeEq
  simp [basesEx, declEx, ancestors]

-- Different provenance: B1 gets x from A; B2 gets x from itself
example : prov basesEx declEx 2 2 "x" = some 0 := by
  unfold prov basesEx declEx ancestors
  simp [basesEx, declEx, ancestors]
example : prov basesEx declEx 2 3 "x" = some 3 := by
  unfold prov basesEx declEx ancestors
  simp [basesEx, declEx, ancestors]

-- Therefore provenance-query is Bases-dependent, hence not shape-respecting
theorem prov_query_not_shape_respecting :
    let q := provIs basesEx declEx 2 "x" 0
    q ∉ ShapeQuerySet nsEx := by
  intro q
  have hshape : shapeEq nsEx 2 3 := by
    unfold nsEx visibleNS shapeEq
    simp [basesEx, declEx, ancestors]
  have hq2 : q 2 = true := by
    dsimp [q, provIs, prov, basesEx, declEx, ancestors]
    simp [basesEx, declEx, ancestors]
  have hq3 : q 3 = false := by
    dsimp [q, provIs, prov, basesEx, declEx, ancestors]
    simp [basesEx, declEx, ancestors]
  have hdiff : q 2 ≠ q 3 := by simp [hq2, hq3]
  have hdep : BasesDependentQuery nsEx q := ⟨2, 3, hshape, hdiff⟩
  exact (outside_shape_iff_bases_dependent nsEx q).2 hdep
...
def protoCheck (ns : Namespace) (req : List AttrName) : SingleQuery :=
  fun T => decide (∀ a, a ∈ req → a ∈ ns T)

-- If two protocol descriptions have the same required-member set, their
-- runtime predicates coincide.
theorem protoCheck_eq_of_same_structure (ns : Namespace) {req₁ req₂ : List AttrName}
    (h : ∀ a : AttrName, a ∈ req₁ ↔ a ∈ req₂) :
    protoCheck ns req₁ = protoCheck ns req₂ := by
  funext T
  have hiff :
      (∀ a : AttrName, a ∈ req₁ → a ∈ ns T) ↔
      (∀ a : AttrName, a ∈ req₂ → a ∈ ns T) := by
    constructor
...
theorem protoCheck_in_shapeQuerySet (ns : Namespace) (req : List AttrName) :
    protoCheck ns req ∈ ShapeQuerySet ns := by
  -- unfold membership in ShapeQuerySet
  show ShapeRespectingSingle ns (protoCheck ns req)
  intro A B hAB
  have hns : ns A = ns B := hAB
  have hiff :
      (∀ a : AttrName, a ∈ req → a ∈ ns A) ↔
      (∀ a : AttrName, a ∈ req → a ∈ ns B) := by simpa [hns]
  simpa [protoCheck, decide_eq_decide] using hiff
...
theorem protocol_alias
    (ns : Namespace) (P Q : Typ)
    (hPQ : ns P = ns Q) :
    ∀ xType, shapeCompatible ns xType P ↔ shapeCompatible ns xType Q := by
  intro xType
  constructor
  · intro h attr hattrQ
    have hattrP : attr ∈ ns P := by simpa [hPQ] using hattrQ
    exact h attr hattrP
  · intro h attr hattrP
    have hattrQ : attr ∈ ns Q := by simpa [hPQ] using hattrP
    exact h attr hattrQ

-- If structural conformance distinguishes P from Q, their namespaces differ.
theorem distinguishable_implies_namespace_diff
    (ns : Namespace) (P Q : Typ)
    (hDist : ∃ xType, shapeCompatible ns xType P ≠ shapeCompatible ns xType Q) :
    ns P ≠ ns Q := by
  intro hEq
  rcases hDist with ⟨xType, hx⟩
  have hAlias := protocol_alias ns P Q hEq xType
  have hSame : shapeCompatible ns xType P = shapeCompatible ns xType Q :=
    propext hAlias
  exact hx hSame
...
def ShapeSetoid (ns : Namespace) : Setoid Typ :=
{ r := fun A B => ns A = ns B
, iseqv := by
...
abbrev ShapeQuot (ns : Namespace) := Quot (ShapeSetoid ns)

-- Shape-respecting ↔ factors through the shape quotient
theorem shapeRespecting_iff_factors (ns : Namespace) (f : Typ → α) :
    ShapeRespecting ns f ↔
      ∃ g : ShapeQuot ns → α, ∀ T, f T = g (Quot.mk _ T) := by
  constructor
  · intro hf
    -- define g via quotient lift
    let g : ShapeQuot ns → α :=
      Quot.lift f (by
        intro A B hAB
        exact hf A B hAB)
    refine ⟨g, ?_⟩
    intro T; rfl
  · rintro ⟨g, hg⟩ A B hAB
    have hq : Quot.mk (ShapeSetoid ns) A = Quot.mk (ShapeSetoid ns) B :=
      Quot.sound hAB
...
theorem capability_requires_bases (ns : Namespace) (q : SingleQuery) :
    BasesDependentQuery ns q → q ∉ ShapeQuerySet ns := by
  intro hdep
  exact (outside_shape_iff_bases_dependent ns q).2 hdep
