...
-- Make decidability of nominal compatibility explicit for this concrete Typ
instance decidableNominalCompatible (B : Bases) (fuel : Nat) (xType T : Typ) :
    Decidable (nominalCompatible B fuel xType T) := by
  unfold nominalCompatible
  infer_instance

-- Modeled `hasattr(obj, a)` as a single-query on namespaces
def pyHasattr (ns : Namespace) (attr : AttrName) : SingleQuery :=
  fun T => decide (attr ∈ ns T)

-- `hasattr` is shape-respecting: it depends only on the namespace
theorem pyHasattr_shape_respecting (ns : Namespace) (attr : AttrName) :
    pyHasattr ns attr ∈ ShapeQuerySet ns := by
  -- unfold the definition of shape-respecting
  dsimp [ShapeQuerySet, ShapeRespectingSingle, pyHasattr]
  intro A B hns
  -- same namespace ⇒ membership is preserved
  dsimp [shapeEq] at hns
  simpa [hns]

-- Modeled `isinstance(obj, T)` as nominal compatibility
def pyIsinstance (B : Bases) (fuel : Nat) (T : Typ) [DecidableEq Typ] :
    SingleQuery :=
  fun xType => decide (nominalCompatible B fuel xType T)
...
theorem pyIsinstance_bases_sensitive :
    ...
    let ns : Namespace := fun _ => ["x"]
    let q : SingleQuery := pyIsinstance B 1 0
    q ∉ ShapeQuerySet ns := by
  classical
  -- instantiate witnesses
  let B : Bases := fun t => if t = 0 then [1] else []
  let ns : Namespace := fun _ => ["x"]
  let q : SingleQuery := pyIsinstance B 1 0
  -- show Bases-dependent: same shape, different query result
  have h_shape : shapeEq ns 0 1 := by
    dsimp [shapeEq, ns]
  have h_diff : q 0 ≠ q 1 := by
    dsimp [q, pyIsinstance, nominalCompatible, ancestors, B]
    native_decide
  have h_dep : BasesDependentQuery ns q := ⟨0, 1, h_shape, h_diff⟩
  exact (outside_shape_iff_bases_dependent ns q).2 h_dep
