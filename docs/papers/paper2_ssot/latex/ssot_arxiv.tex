...
All theorems machine-checked in Lean 4 (1,811 lines across 13 files, 0 \texttt{sorry} placeholders). Empirical validation: 13 case studies from production bioimage analysis platform (OpenHCS, 45K LoC), mean DOF reduction 14.2x.
...
This paper proves that certain programming languages are \emph{incapable} of achieving the Single Source of Truth (SSOT) principle for structural facts. All results are machine-checked in Lean 4 (1,811 lines across 13 files, 0 \texttt{sorry} placeholders).
...
By Definition~\ref{def:encodes}, the program is incorrect. Therefore, all $k$ locations must be updated, and $k$ is the minimum. \(\blacksquare\)
\end{proof}
...
\begin{proof}
Each independent location can hold a different value. By Definition~\ref{def:independent}, no constraint forces agreement between independent locations. Therefore, $k$ independent locations can hold $k$ distinct values. The program may compile and run, but it encodes inconsistent specifications. \(\blacksquare\)
\end{proof}
...
Therefore, DOF = 1 is the unique value that avoids both underspecification and inconsistency risk. \(\blacksquare\)
\end{proof}
...
The program is correct after 1 edit. Therefore, effective modification complexity is 1. \(\blacksquare\)
\end{proof}
...
Therefore, $L_d$ is not independent of $L_s$, and does not contribute to DOF. \(\blacksquare\)
\end{proof}
...
\begin{proof}
Let $L_s$ be the non-derived encoding. All other encodings $L_1, \ldots, L_k$ are derived from $L_s$. By Theorem~\ref{thm:derivation-excludes}, none of $L_1, \ldots, L_k$ contribute to DOF. Only $L_s$ contributes. Therefore, $\text{DOF}(C, F) = 1$. \(\blacksquare\)
\end{proof}
...
Therefore, derivation for structural facts must occur at definition time ($t_D = t_{\text{fix}}$). \(\blacksquare\)
\end{proof}
...
Contrapositive: If a language lacks definition-time hooks, SSOT for structural facts is impossible. \(\blacksquare\)
\end{proof}
...
Without verifiable SSOT, the programmer cannot confirm SSOT holds. They must trust that their code is correct without runtime confirmation. Bugs in derivation logic go undetected. \(\blacksquare\)
\end{proof}
...
\begin{remark}[Achievable vs. Verifiable SSOT]\label{rem:achievable-vs-verifiable}
This paper concerns \emph{verifiable} SSOT: the property that a program can confirm its own DOF = 1 at runtime. A language may \emph{achieve} SSOT (through compile-time mechanisms) without being able to \emph{verify} it. Unverifiable SSOT is weaker because:
\begin{enumerate}
\tightlist
\item Bugs in derivation logic are undetectable at runtime
\item The programmer must trust rather than confirm
\item Refactoring can break SSOT silently
\end{enumerate}
All impossibility theorems in this paper concern verifiable SSOT.
\end{remark}
...
Therefore, the requirements are independent. \(\blacksquare\)
\end{proof}
...
A language $L$ enables SSOT for structural facts if and only if:
\begin{enumerate}
\tightlist
\item $L$ provides definition-time hooks, AND
\item $L$ provides introspectable derivation results, AND
\item $L$ provides structural modification capability (can add/modify class attributes at definition time)
\end{enumerate}
\end{theorem}

\begin{proof}
$(\Rightarrow)$ \textbf{Necessity:} Suppose $L$ enables SSOT for structural facts.
\begin{itemize}
\tightlist
...
\item For structural facts that require derived attributes (e.g., registry entries, computed methods), $L$ must be able to add structure at definition time
\end{itemize}

$(\Leftarrow)$ \textbf{Sufficiency:} Suppose $L$ provides all three capabilities.
\begin{itemize}
\tightlist
\item Definition-time hooks enable derivation at the right moment (when structure is fixed)
\item Structural modification enables adding derived attributes to classes
...
The if-and-only-if follows. \(\blacksquare\)
\end{proof}

\begin{corollary}[SSOT-Complete Languages]\label{cor:ssot-complete}
A language is \emph{SSOT-complete} iff it satisfies all three requirements. A language is \emph{SSOT-incomplete} otherwise.
\end{corollary}

\begin{remark}[Partial SSOT]\label{rem:partial-ssot}
A language with DEF + INTRO but limited STRUCT (e.g., Ruby) can achieve SSOT for \emph{read-only} structural facts (querying inheritance, enumerating methods) but not for facts requiring structural modification (adding computed attributes, auto-registering handlers). We call this \emph{read-only SSOT}.
\end{remark}
...
\textbf{Counterargument:} ``If generation is mandatory and deterministic, functional equivalence to derivation holds.''

\textbf{Response:} The file system introduces failure modes \emph{not present in in-memory derivation}:
\begin{itemize}
\tightlist
\item \textbf{Staleness:} Source changed but generation not re-run. The file system can be in a state where $E_1$ and $E_2$ encode different values.
\item \textbf{Independent modification:} $E_2$ can be hand-edited, bypassing the source. The language cannot prevent this.
\item \textbf{Build system failure:} Generation step fails silently, stale $E_2$ is compiled. Tests may pass with old behavior.
\item \textbf{Cache invalidation:} Build system incorrectly caches $E_2$, skipping regeneration.
\end{itemize}

In-memory derivation has none of these failure modes: the derived value exists \emph{only} as a consequence of the source definition, with no intermediate checkpoint that could diverge. The language runtime guarantees the relationship.

Contrast with Python, where the registry entry exists only in memory, created by the class statement itself. There is no second file. DOF $= 1$. \(\blacksquare\)
\end{proof}
...
Without introspection, the program cannot verify DOF $= 1$. SSOT may hold but cannot be confirmed. \(\blacksquare\)
\end{proof}
...
\subsubsection{Rust: Unverifiable SSOT}

Rust's procedural macros are compile-time and opaque:

\textbf{DEF:} Partial. Procedural macros execute at compile time. This \emph{is} a form of definition-time hook---code runs when the struct/impl is compiled.

\textbf{INTRO:} $\times$. No runtime type introspection. \texttt{std::any::TypeId} provides minimal information. The program cannot query what the macro generated.

\textbf{STRUCT:} $\times$. Cannot modify type structure at runtime.

\textbf{HIER:} $\times$. Cannot enumerate trait implementers.

\textbf{Critical distinction:} Rust's macros may \emph{achieve} SSOT (the derivation happens at compile time), but cannot \emph{verify} it (the program cannot confirm the derivation occurred). Our theorems concern \emph{verifiable} SSOT. Unverifiable SSOT is weaker: it might work, but bugs in derivation logic are undetectable at runtime.
...
\begin{proof}
By exhaustive evaluation. We checked all 10 mainstream languages against the four criteria. Only Python satisfies all four. The evaluation is complete---no mainstream language is omitted. \(\blacksquare\)
\end{proof}
...
Ruby achieves \emph{read-only SSOT} (Remark~\ref{rem:partial-ssot}): it can derive facts from structural queries but cannot add computed attributes during definition. This is sufficient for some SSOT use cases but not all.

\begin{theorem}[Three-Language Theorem]\label{thm:three-lang}
Exactly three languages in common use satisfy complete SSOT requirements (DEF $\land$ INTRO $\land$ STRUCT): Python, Common Lisp (CLOS), and Smalltalk.
\end{theorem}

\begin{proof}
By exhaustive evaluation of mainstream and notable non-mainstream languages against the three requirements of Theorem~\ref{thm:ssot-iff}. Python, CLOS, and Smalltalk satisfy all three. Ruby satisfies DEF and INTRO but only partially STRUCT, achieving read-only SSOT. All other evaluated languages fail at least two requirements. \(\blacksquare\)
\end{proof}
...
The number of derived locations $k$ may grow with codebase size, but the number of \emph{manual} edits remains 1. Therefore, $M_{\text{effective}}(C, \delta_F) = O(1)$. \(\blacksquare\)
\end{proof}
...
Therefore, $M_{\text{effective}}(C, \delta_F) = \Omega(n)$. \(\blacksquare\)
\end{proof}
...
As $n \to \infty$, the ratio $\to \infty$. The gap is unbounded. \(\blacksquare\)
\end{proof}
...
\begin{proof}
Choose $n = k$. Then $M_{\text{incomplete}}(n) = n = k$ and $M_{\text{complete}} = 1$. The reduction factor is $k/1 = k$. \(\blacksquare\)
\end{proof}
...
\begin{proof}
Each modification costs $O(1)$ with SSOT and $O(n)$ without. Over $m$ modifications, total cost is $m \cdot O(1) = O(m)$ with SSOT and $m \cdot O(n) = O(mn)$ without. \(\blacksquare\)
\end{proof}
...
All theorems are machine-checked in Lean 4 (1,811 lines across 13 files, 0 \texttt{sorry} placeholders). Complete source available at: \texttt{proofs/ssot/}.
...
\begin{tabular}{lcc}
\toprule
\textbf{File} & \textbf{Lines} & \textbf{Defs/Thms} \\
\midrule
Basic.lean & 47 & 3 \\
SSOT.lean & 37 & 4 \\
Derivation.lean & 41 & 3 \\
Requirements.lean & 112 & 12 \\
Completeness.lean & 167 & 17 \\
Bounds.lean & 76 & 10 \\
Languages.lean & 108 & 16 \\
CaseStudies.lean & 148 & 22 \\
Foundations.lean & 364 & 37 \\
LangPython.lean & 209 & 21 \\
LangRust.lean & 184 & 15 \\
LangStatic.lean & 163 & 11 \\
LangEvaluation.lean & 155 & 8 \\
\midrule
\textbf{Total} & \textbf{1,811} & \textbf{179} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{All 179 definitions/theorems compile without \texttt{sorry} placeholders.} The proofs can be verified by running \texttt{lake build} in the \texttt{proofs/ssot/} directory. Every theorem in the paper corresponds to a machine-checked proof.
