...
\begin{proof}
By Theorem~\ref{thm:dof-one-coherence}, DOF = 1 guarantees coherence. Coherence means all encodings hold the same value. Therefore, the value of $F$ is uniquely determined by the single source. \qed
\end{proof}
...
Coherence restoration requires 1 edit. \qed
\end{proof}
...
Therefore, DOF = 1 is uniquely coherent. This is epistemic necessity, not design choice. \qed
\end{proof}
...
\begin{proof}
Direct application of Theorem~\ref{thm:dof-gt-one-incoherence}. With DOF $> 1$, independent locations can be modified separately, reaching states where they disagree. \qed
\end{proof}
...
There is no reachable state where $L_s = v'$ and $L_d = f(v)$ with $v' \neq v$. Divergence is impossible. Therefore, $L_d$ does not contribute to DOF. \qed
\end{proof}
...
\begin{proof}
Let $L_s$ be the non-derived encoding. All other encodings $L_1, \ldots, L_k$ are derived from $L_s$. By Theorem~\ref{thm:derivation-excludes}, none can diverge. Only $L_s$ is independent. Therefore, $\text{DOF}(C, F) = 1$, and by Theorem~\ref{thm:dof-one-coherence}, coherence is guaranteed. \qed
\end{proof}
