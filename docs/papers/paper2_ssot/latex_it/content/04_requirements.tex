...
We now derive the capabilities necessary and sufficient for encoding systems to achieve DOF = 1 (optimal encoding rate). These requirements are \emph{information-theoretic necessities}---properties that any encoding system must have to guarantee coherence under modification, regardless of implementation domain.
...
\subsection{The Realizability Question}\label{sec:realizability-question}

Given that DOF = 1 is the unique optimal encoding rate (Theorem~\ref{thm:dof-optimal}), a natural question arises: \emph{What must an encoding system provide for DOF = 1 to be realizable?}

An encoding system consists of:
\begin{itemize}
\tightlist
\item \textbf{Locations}: Sites where facts can be encoded
\item \textbf{Encodings}: Values stored at locations
\item \textbf{Modifications}: Operations that change encodings
\item \textbf{Derivation mechanism}: Rules determining how some locations are computed from others
\end{itemize}

For DOF = 1 to hold, exactly one location must be independent (the \emph{source}), and all others must be \emph{derived}---automatically computed from the source such that divergence is impossible.

We prove that two properties are necessary and sufficient for DOF = 1 realizability:
\begin{enumerate}
\item \textbf{Causal update propagation}: Changes to the source automatically trigger updates to derived locations
\item \textbf{Provenance observability}: The system supports queries about derivation structure (what is derived from what)
\end{enumerate}

These are \textbf{encoder properties}, not implementation details. They determine whether an encoding system can achieve the optimal rate.
...
\item Derived locations have not been updated (they may not exist, or hold stale values)
