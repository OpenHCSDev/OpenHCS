...
\item
  ...
  Adapters eliminate the retrofit exception (Theorem 2.10j). Shape-based
  typing provides a strict subset of capabilities when inheritance exists.
\item
...
universe of possible typing systems}:

\begin{itemize}
\tightlist
\item Theorem 3.13 proves no model over $(N, S)$ alone can compute provenance.
\item Theorem 3.19 proves the capability gap is derived from information structure.
\item Theorem 3.24 proves no algorithm can achieve better than $\Omega(n)$
  for duck typing error localization.
\end{itemize}

These results are information-theoretic and apply to any typing system
that discards the Bases axis.
...
\textbf{Corollary 3.43k (Scope Verification).} Any claim that
``$B = \emptyset$ is the common case'' requires exhibiting a non-trivial
production codebase using none of the tooling in Theorem 3.43i.
...
A critical distinction: \textbf{discipline optimality} (which typing
paradigm has more capabilities) is independent of \textbf{migration
optimality} (whether migrating an existing codebase is beneficial).
...
\textbf{Corollary 7.1 (Prior Work Comparison).} Any claim that
``this was already proven'' requires exhibiting a publication
scoring $\geq$1/5 in Table 7.5.3. Our survey found no such publication,
...
\textbf{Rows not included:} Earlier versions included ``Retrofit / external
types $\rightarrow$ Structural'' and ``Small scripts $\rightarrow$ Duck.'' These were
removed because adapters make nominal typing available in retrofit contexts
(Theorem 2.10j), and duck typing lacks coherence (Theorem 2.10d).

The methodology states: \textbf{if $B \neq \emptyset$, nominal typing
is the capability-maximizing choice.} The decision is determined by
whether the language has inheritance, not by project size or convenience.
...
nominal typing with explicit adapters for external types.

\subsubsection{Summary of Results}\label{summary-of-results}

Typing discipline selection has traditionally been treated as a matter of
style or preference. Our formal analysis establishes it as a matter of
capability: different disciplines provide different capabilities, and
these differences are mathematically derivable.
