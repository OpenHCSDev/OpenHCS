...
\section{Typing Discipline Selection for Object-Oriented
Systems}\label{typing-discipline-selection-for-object-oriented-systems}

\textbf{A Formal Methodology with Empirical Validation}
...
Ω(n) inspections---proved by adversary argument. - These theorems admit
...
O(k) vs Ω(n) complexity separation with adversary-based lower bound
...
\texttt{@auto\_create\_decorator} → \texttt{@global\_pipeline\_config}
...
\item
  \textbf{When B ≠ ∅, shape-based typing is wrong.} Nominal typing
...
\subsubsection{1.2 Roadmap}\label{roadmap}
...
\subsubsection{2.2 The type() Theorem}\label{the-type-theorem}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{C }\OperatorTok{=} \BuiltInTok{type}\NormalTok{(name, bases, namespace)}
\ControlFlowTok{assert}\NormalTok{ C.}\VariableTok{\_\_name\_\_} \OperatorTok{==}\NormalTok{ name}
\ControlFlowTok{assert}\NormalTok{ C.\_\_bases\_\_ }\OperatorTok{==}\NormalTok{ bases}
\ControlFlowTok{assert} \BuiltInTok{all}\NormalTok{(namespace[k] }\OperatorTok{==} \BuiltInTok{getattr}\NormalTok{(C, k) }\ControlFlowTok{for}\NormalTok{ k }\KeywordTok{in}\NormalTok{ namespace)}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{2.3 C3 Linearization (Prior
...
\subsubsection{2.4 Abstract Class System
...
\textbf{Remark (Go Embedding ≠ Inheritance).} Go's struct embedding
...
\item
  ...
  \item
    ``Structure ≠ semantics'' would require nominal identity via
    inheritance
  \item
...
\textbf{Theorem 2.10j (Protocol Is Strictly Dominated When B ≠ ∅).} In
...
No declaration → no event. No event → no hook point. 5. Therefore,
...
(O(∞) in open systems) & Requires registration \\
...
\subsubsection{2.5 The Axis Lattice
...
\subsection{3. Universal Dominance}\label{universal-dominance}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Hypothetical minimal class constructor}
\KeywordTok{def}\NormalTok{ type\_minimal(namespace: }\BuiltInTok{dict}\NormalTok{) }\OperatorTok{{-}\textgreater{}} \BuiltInTok{type}\NormalTok{:}
    \CommentTok{"""Create a class from namespace only."""}
    \ControlFlowTok{return} \BuiltInTok{type}\NormalTok{(}\StringTok{""}\NormalTok{, (), namespace)}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{3.1 The Absolute Claim}\label{the-absolute-claim}
...
Our O(1) vs Ω(n) complexity claim requires proving that Ω(n) is a
...
\textbf{Corollary 3.27 (Lower Bound Is Tight).} The Ω(n) lower bound for
...
\subsubsection{3.3 Summary: The Unarguable
...
\textbf{3.24 (Lower Bound)} & Duck typing requires Ω(n) inspections &
...
\subsubsection{3.5 Bulletproof Theorems: Closing All Attack
...
production example demonstrating that structural identity ≠ semantic
identity.
...
precedes which? → \emph{Conflict resolution} (C3 linearization selects
based on MRO order) 2. \textbf{Membership}: Is element X in the list? →
\emph{Enumeration} (subtype iff in some type's MRO) 3. \textbf{Element
identity}: Which specific element? → \emph{Provenance} and \emph{type
...
via: - Ancestor membership: is \(T \in \text{ancestors}(A)\)? → covered
by \textbf{provenance} - Subtype enumeration: what are all
\(T : T <: A\)? → covered by \textbf{enumeration} - MRO position: which
type wins for attribute \(a\)? → covered by
...
sites (1 additional parameter × N calls) - Identity: Must maintain
...
\textbf{Corollary 3.43j (B=∅ Is Exceptional).} The \(B = \emptyset\)
...
attributeCheck\} 3. Shape ⊂ Nominal (strict subset) 4. Declaration cost:
...
Nominal (dominance, universal) 2. If \(B = \emptyset\) → select Shape
...
\subsubsection{3.6 Summary: Attack Surface
...
\item
  ...
  semantics'' nor ``structure ≠ semantics'')
\item
...
\subsection{4. Core Theorems}\label{core-theorems}

\subsubsection{4.1 The Error Localization
...
\subsubsection{4.2 The Information Scattering
...
\subsection{5. Case Studies: Applying the
Methodology}\label{case-studies-applying-the-methodology}

\subsubsection{5.1 Empirical Validation
...
\subsubsection{5.2 Case Study 1: Structurally Identical, Semantically
Distinct
...
\KeywordTok{class}\NormalTok{ WellFilterConfig:}
    \CommentTok{"""Pipeline{-}level scope."""}
\NormalTok{    well\_filter: Optional[Union[List[}\BuiltInTok{str}\NormalTok{], }\BuiltInTok{str}\NormalTok{, }\BuiltInTok{int}\NormalTok{]] }\OperatorTok{=} \VariableTok{None}
\NormalTok{    well\_filter\_mode: WellFilterMode }\OperatorTok{=}\NormalTok{ WellFilterMode.INCLUDE}

\AttributeTok{@dataclass}\NormalTok{(frozen}\OperatorTok{=}\VariableTok{True}\NormalTok{)}
\KeywordTok{class}\NormalTok{ PathPlanningConfig(WellFilterConfig):}
    \CommentTok{"""Pipeline{-}level path configuration."""}
\NormalTok{    output\_dir\_suffix: }\BuiltInTok{str} \OperatorTok{=} \StringTok{"\_openhcs"}
\NormalTok{    sub\_dir: }\BuiltInTok{str} \OperatorTok{=} \StringTok{"images"}  \CommentTok{\# Pipeline default}

\AttributeTok{@dataclass}\NormalTok{(frozen}\OperatorTok{=}\VariableTok{True}\NormalTok{)}
\KeywordTok{class}\NormalTok{ StepWellFilterConfig(WellFilterConfig):}
    \CommentTok{"""Step{-}level scope marker."""}
    \ControlFlowTok{pass}  \CommentTok{\# ZERO new fields. Structurally identical to WellFilterConfig.}

\AttributeTok{@dataclass}\NormalTok{(frozen}\OperatorTok{=}\VariableTok{True}\NormalTok{)}
\KeywordTok{class}\NormalTok{ StepMaterializationConfig(StepWellFilterConfig, PathPlanningConfig):}
    \CommentTok{"""Step{-}level materialization."""}
\NormalTok{    sub\_dir: }\BuiltInTok{str} \OperatorTok{=} \StringTok{"checkpoints"}  \CommentTok{\# Step default OVERRIDES pipeline default}
\NormalTok{    enabled: }\BuiltInTok{bool} \OperatorTok{=} \VariableTok{False}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{StepMaterializationConfig.\_\_mro\_\_ }\OperatorTok{=}\NormalTok{ (}
\NormalTok{    StepMaterializationConfig,  }\CommentTok{\# Step scope}
\NormalTok{    StepWellFilterConfig,       }\CommentTok{\# Step scope marker (NO FIELDS!)}
\NormalTok{    PathPlanningConfig,         }\CommentTok{\# Pipeline scope}
\NormalTok{    WellFilterConfig,           }\CommentTok{\# Pipeline scope}
    \BuiltInTok{object}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\texttt{StepMaterializationConfig.sub\_dir\ =\ "checkpoints"} →
\textbf{step-level value} 2.
\texttt{PathPlanningConfig.sub\_dir\ =\ "images"} → pipeline-level value
(shadowed)
...
\tightlist
\item
  ``Is this config pipeline-level or step-level?'' → Determines
  resolution priority
\item
  ``Which type in the MRO provided \texttt{sub\_dir}?'' → Provenance for
  debugging
\item
  ``Can I use \texttt{isinstance(config,\ StepWellFilterConfig)}?'' →
...
\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{isinstance}\NormalTok{(config, StepWellFilterConfig)  }\CommentTok{\# Scope check: O(1)}
\BuiltInTok{type}\NormalTok{(config).\_\_mro\_\_                       }\CommentTok{\# Full provenance chain: O(1)}
\BuiltInTok{type}\NormalTok{(config).\_\_mro\_\_.index(StepWellFilterConfig)  }\CommentTok{\# MRO position: O(k)}
\end{Highlighting}
\end{Shaded}
...
identity via \texttt{isinstance()} - Theorem 4.3: O(1) vs Ω(n)
...
\KeywordTok{class}\NormalTok{ OptionalDataclassInfo(ParameterInfoBase):}
\NormalTok{    widget\_creation\_type: }\BuiltInTok{str} \OperatorTok{=} \StringTok{"OPTIONAL\_NESTED"}

    \AttributeTok{@staticmethod}
    \KeywordTok{def}\NormalTok{ matches(param\_type: Type) }\OperatorTok{{-}\textgreater{}} \BuiltInTok{bool}\NormalTok{:}
        \ControlFlowTok{return}\NormalTok{ is\_optional(param\_type) }\KeywordTok{and}\NormalTok{ is\_dataclass(inner\_type(param\_type))}

\AttributeTok{@dataclass}
\KeywordTok{class}\NormalTok{ DirectDataclassInfo(ParameterInfoBase):}
\NormalTok{    widget\_creation\_type: }\BuiltInTok{str} \OperatorTok{=} \StringTok{"NESTED"}

    \AttributeTok{@staticmethod}
    \KeywordTok{def}\NormalTok{ matches(param\_type: Type) }\OperatorTok{{-}\textgreater{}} \BuiltInTok{bool}\NormalTok{:}
        \ControlFlowTok{return}\NormalTok{ is\_dataclass(param\_type)}

\AttributeTok{@dataclass}
\KeywordTok{class}\NormalTok{ GenericInfo(ParameterInfoBase):}
    \AttributeTok{@staticmethod}
    \KeywordTok{def}\NormalTok{ matches(param\_type: Type) }\OperatorTok{{-}\textgreater{}} \BuiltInTok{bool}\NormalTok{:}
        \ControlFlowTok{return} \VariableTok{True}  \CommentTok{\# Fallback}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{5.4 Case Study 3: MemoryTypeConverter
Dispatch}\label{case-study-3-memorytypeconverter-dispatch}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# 6 converter classes auto{-}generated at module load}
\NormalTok{\_CONVERTERS }\OperatorTok{=}\NormalTok{ \{}
\NormalTok{    mem\_type: }\BuiltInTok{type}\NormalTok{(}
        \SpecialStringTok{f"}\SpecialCharTok{\{}\NormalTok{mem\_type}\SpecialCharTok{.}\NormalTok{value}\SpecialCharTok{.}\NormalTok{capitalize()}\SpecialCharTok{\}}\SpecialStringTok{Converter"}\NormalTok{,  }\CommentTok{\# name}
\NormalTok{        (MemoryTypeConverter,),                      }\CommentTok{\# bases}
\NormalTok{        \_TYPE\_OPERATIONS[mem\_type]                   }\CommentTok{\# namespace}
\NormalTok{    )()}
    \ControlFlowTok{for}\NormalTok{ mem\_type }\KeywordTok{in}\NormalTok{ MemoryType}
\NormalTok{\}}

\KeywordTok{def}\NormalTok{ convert\_memory(data, source\_type: }\BuiltInTok{str}\NormalTok{, target\_type: }\BuiltInTok{str}\NormalTok{, gpu\_id: }\BuiltInTok{int}\NormalTok{):}
\NormalTok{    source\_enum }\OperatorTok{=}\NormalTok{ MemoryType(source\_type)}
\NormalTok{    converter }\OperatorTok{=}\NormalTok{ \_CONVERTERS[source\_enum]  }\CommentTok{\# O(1) lookup by type}
\NormalTok{    method }\OperatorTok{=} \BuiltInTok{getattr}\NormalTok{(converter, }\SpecialStringTok{f"to\_}\SpecialCharTok{\{}\NormalTok{target\_type}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}
    \ControlFlowTok{return}\NormalTok{ method(data, gpu\_id)}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{5.5 Case Study 4: Polymorphic
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{class}\NormalTok{ StreamingConfig(StreamingDefaults, ABC):}
    \AttributeTok{@property}
    \AttributeTok{@abstractmethod}
    \KeywordTok{def}\NormalTok{ backend(}\VariableTok{self}\NormalTok{) }\OperatorTok{{-}\textgreater{}}\NormalTok{ Backend: }\ControlFlowTok{pass}

\CommentTok{\# Factory{-}generated concrete types}
\NormalTok{NapariStreamingConfig }\OperatorTok{=}\NormalTok{ create\_streaming\_config(}
\NormalTok{    viewer\_name}\OperatorTok{=}\StringTok{\textquotesingle{}napari\textquotesingle{}}\NormalTok{, port}\OperatorTok{=}\DecValTok{5555}\NormalTok{, backend}\OperatorTok{=}\NormalTok{Backend.NAPARI\_STREAM)}
\NormalTok{FijiStreamingConfig }\OperatorTok{=}\NormalTok{ create\_streaming\_config(}
\NormalTok{    viewer\_name}\OperatorTok{=}\StringTok{\textquotesingle{}fiji\textquotesingle{}}\NormalTok{, port}\OperatorTok{=}\DecValTok{5565}\NormalTok{, backend}\OperatorTok{=}\NormalTok{Backend.FIJI\_STREAM)}

\CommentTok{\# Orchestrator dispatch}
\ControlFlowTok{if} \BuiltInTok{isinstance}\NormalTok{(config, StreamingConfig):}
\NormalTok{    registry.get\_or\_create\_tracker(config.port, config.viewer\_type)}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{5.6 Case Study 5: Migration from Duck to Nominal Typing
(PR
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# BEFORE: Duck typing dispatch (scattered across 47 call sites)}
\ControlFlowTok{if} \BuiltInTok{hasattr}\NormalTok{(widget, }\StringTok{\textquotesingle{}isChecked\textquotesingle{}}\NormalTok{):}
    \ControlFlowTok{return}\NormalTok{ widget.isChecked()}
\ControlFlowTok{elif} \BuiltInTok{hasattr}\NormalTok{(widget, }\StringTok{\textquotesingle{}currentText\textquotesingle{}}\NormalTok{):}
    \ControlFlowTok{return}\NormalTok{ widget.currentText()}
\CommentTok{\# ... 45 more cases}

\CommentTok{\# AFTER: Nominal ABC (single definition point)}
\KeywordTok{class}\NormalTok{ AbstractFormWidget(ABC):}
    \AttributeTok{@abstractmethod}
    \KeywordTok{def}\NormalTok{ get\_value(}\VariableTok{self}\NormalTok{) }\OperatorTok{{-}\textgreater{}}\NormalTok{ Any: }\ControlFlowTok{pass}

\CommentTok{\# Error detection: attribute typos caught at import time, not user interaction time}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{5.7 Case Study 6:
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{class}\NormalTok{ AutoRegisterMeta(ABCMeta):}
    \KeywordTok{def} \FunctionTok{\_\_new\_\_}\NormalTok{(mcs, name, bases, attrs, registry\_config}\OperatorTok{=}\VariableTok{None}\NormalTok{):}
\NormalTok{        new\_class }\OperatorTok{=} \BuiltInTok{super}\NormalTok{().}\FunctionTok{\_\_new\_\_}\NormalTok{(mcs, name, bases, attrs)}

        \CommentTok{\# Skip abstract classes (nominal check via \_\_abstractmethods\_\_)}
        \ControlFlowTok{if} \BuiltInTok{getattr}\NormalTok{(new\_class, }\StringTok{\textquotesingle{}\_\_abstractmethods\_\_\textquotesingle{}}\NormalTok{, }\VariableTok{None}\NormalTok{):}
            \ControlFlowTok{return}\NormalTok{ new\_class}

        \CommentTok{\# Register using type as value}
\NormalTok{        key }\OperatorTok{=}\NormalTok{ mcs.\_get\_registration\_key(name, new\_class, registry\_config)}
\NormalTok{        registry\_config.registry\_dict[key] }\OperatorTok{=}\NormalTok{ new\_class}
        \ControlFlowTok{return}\NormalTok{ new\_class}

\CommentTok{\# Usage: Define class $\textbackslash{}rightarrow$ auto{-}registered}
\KeywordTok{class}\NormalTok{ ImageXpressHandler(MicroscopeHandler, metaclass}\OperatorTok{=}\NormalTok{MicroscopeHandlerMeta):}
\NormalTok{    \_microscope\_type }\OperatorTok{=} \StringTok{\textquotesingle{}imagexpress\textquotesingle{}}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{5.8 Case Study 7: Five-Stage Type
...
\begin{Shaded}
\begin{Highlighting}[]
\AttributeTok{@auto\_create\_decorator}
\AttributeTok{@dataclass}\NormalTok{(frozen}\OperatorTok{=}\VariableTok{True}\NormalTok{)}
\KeywordTok{class}\NormalTok{ GlobalPipelineConfig:}
\NormalTok{    num\_workers: }\BuiltInTok{int} \OperatorTok{=} \DecValTok{1}
\end{Highlighting}
\end{Shaded}
...
\texttt{create\_global\_default\_decorator(GlobalPipelineConfig)} →
...
\begin{Shaded}
\begin{Highlighting}[]
\AttributeTok{@global\_pipeline\_config}\NormalTok{(inherit\_as\_none}\OperatorTok{=}\VariableTok{True}\NormalTok{)}
\AttributeTok{@dataclass}\NormalTok{(frozen}\OperatorTok{=}\VariableTok{True}\NormalTok{)}
\KeywordTok{class}\NormalTok{ PathPlanningConfig(WellFilterConfig):}
\NormalTok{    output\_dir\_suffix: }\BuiltInTok{str} \OperatorTok{=} \StringTok{""}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{5.9 Case Study 8: Dual-Axis Resolution
Algorithm}\label{case-study-8-dual-axis-resolution-algorithm}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{def}\NormalTok{ resolve\_field\_inheritance(obj, field\_name, scope\_stack):}
\NormalTok{    mro }\OperatorTok{=}\NormalTok{ [normalize\_type(T) }\ControlFlowTok{for}\NormalTok{ T }\KeywordTok{in} \BuiltInTok{type}\NormalTok{(obj).\_\_mro\_\_]}

    \ControlFlowTok{for}\NormalTok{ scope }\KeywordTok{in}\NormalTok{ scope\_stack:  }\CommentTok{\# X{-}axis: context hierarchy}
        \ControlFlowTok{for}\NormalTok{ mro\_type }\KeywordTok{in}\NormalTok{ mro:    }\CommentTok{\# Y{-}axis: class hierarchy}
\NormalTok{            config }\OperatorTok{=}\NormalTok{ get\_config\_at\_scope(scope, mro\_type)}
            \ControlFlowTok{if}\NormalTok{ config }\KeywordTok{and} \BuiltInTok{hasattr}\NormalTok{(config, field\_name):}
\NormalTok{                value }\OperatorTok{=} \BuiltInTok{getattr}\NormalTok{(config, field\_name)}
                \ControlFlowTok{if}\NormalTok{ value }\KeywordTok{is} \KeywordTok{not} \VariableTok{None}\NormalTok{:}
                    \ControlFlowTok{return}\NormalTok{ (value, scope, mro\_type)  }\CommentTok{\# Provenance tuple}
    \ControlFlowTok{return}\NormalTok{ (}\VariableTok{None}\NormalTok{, }\VariableTok{None}\NormalTok{, }\VariableTok{None}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{5.10 Case Study 9: Custom isinstance()
Implementation}\label{case-study-9-custom-isinstance-implementation}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{class}\NormalTok{ GlobalConfigMeta(}\BuiltInTok{type}\NormalTok{):}
    \KeywordTok{def} \FunctionTok{\_\_instancecheck\_\_}\NormalTok{(cls, instance):}
        \CommentTok{\# Virtual base class check}
        \ControlFlowTok{if} \BuiltInTok{hasattr}\NormalTok{(instance.\_\_class\_\_, }\StringTok{\textquotesingle{}\_is\_global\_config\textquotesingle{}}\NormalTok{):}
            \ControlFlowTok{return}\NormalTok{ instance.\_\_class\_\_.\_is\_global\_config}
        \ControlFlowTok{return} \BuiltInTok{super}\NormalTok{().}\FunctionTok{\_\_instancecheck\_\_}\NormalTok{(instance)}

\CommentTok{\# Usage: isinstance(config, GlobalConfigBase) returns True}
\CommentTok{\# even if config doesn\textquotesingle{}t inherit from GlobalConfigBase}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{5.11 Case Study 10: Dynamic Interface
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{class}\NormalTok{ DynamicInterfaceMeta(ABCMeta):}
\NormalTok{    \_generated\_interfaces: Dict[}\BuiltInTok{str}\NormalTok{, Type] }\OperatorTok{=}\NormalTok{ \{\}}

    \AttributeTok{@classmethod}
    \KeywordTok{def}\NormalTok{ get\_or\_create\_interface(mcs, interface\_name: }\BuiltInTok{str}\NormalTok{) }\OperatorTok{{-}\textgreater{}}\NormalTok{ Type:}
        \ControlFlowTok{if}\NormalTok{ interface\_name }\KeywordTok{not} \KeywordTok{in}\NormalTok{ mcs.\_generated\_interfaces:}
            \CommentTok{\# Generate pure nominal type}
\NormalTok{            interface }\OperatorTok{=} \BuiltInTok{type}\NormalTok{(interface\_name, (ABC,), \{\})}
\NormalTok{            mcs.\_generated\_interfaces[interface\_name] }\OperatorTok{=}\NormalTok{ interface}
        \ControlFlowTok{return}\NormalTok{ mcs.\_generated\_interfaces[interface\_name]}

\CommentTok{\# Runtime usage}
\NormalTok{IStreamingConfig }\OperatorTok{=}\NormalTok{ DynamicInterfaceMeta.get\_or\_create\_interface(}\StringTok{"IStreamingConfig"}\NormalTok{)}
\KeywordTok{class}\NormalTok{ NapariConfig(StreamingConfig, IStreamingConfig): }\ControlFlowTok{pass}

\CommentTok{\# Later: isinstance(config, IStreamingConfig) $\textbackslash{}rightarrow$ True}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{5.12 Case Study 11: Framework Detection via Sentinel
Type}\label{case-study-11-framework-detection-via-sentinel-type}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Framework config uses sentinel type as registry key}
\NormalTok{\_FRAMEWORK\_CONFIG }\OperatorTok{=} \BuiltInTok{type}\NormalTok{(}\StringTok{"\_FrameworkConfigSentinel"}\NormalTok{, (), \{\})()}

\CommentTok{\# Detection: check if sentinel is registered}
\KeywordTok{def}\NormalTok{ has\_framework\_config():}
    \ControlFlowTok{return}\NormalTok{ \_FRAMEWORK\_CONFIG }\KeywordTok{in}\NormalTok{ GlobalRegistry.configs}

\CommentTok{\# Alternative approaches fail:}
\CommentTok{\# hasattr(module, \textquotesingle{}\_FRAMEWORK\_CONFIG\textquotesingle{}) $\textbackslash{}rightarrow$ fragile, module probing}
\CommentTok{\# \textquotesingle{}framework\textquotesingle{} in config\_names $\textbackslash{}rightarrow$ string{-}based, no type safety}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{5.13 Case Study 12: Dynamic Method
Injection}\label{case-study-12-dynamic-method-injection}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{def}\NormalTok{ inject\_conversion\_methods(target\_type: Type, methods: Dict[}\BuiltInTok{str}\NormalTok{, Callable]):}
    \CommentTok{"""Inject methods into a type\textquotesingle{}s namespace at runtime."""}
    \ControlFlowTok{for}\NormalTok{ method\_name, method\_impl }\KeywordTok{in}\NormalTok{ methods.items():}
        \BuiltInTok{setattr}\NormalTok{(target\_type, method\_name, method\_impl)}

\CommentTok{\# Usage: Inject GPU conversion methods into MemoryType converters}
\NormalTok{inject\_conversion\_methods(NumpyConverter, \{}
    \StringTok{\textquotesingle{}to\_cupy\textquotesingle{}}\NormalTok{: }\KeywordTok{lambda} \VariableTok{self}\NormalTok{, data, gpu: cupy.asarray(data, gpu),}
    \StringTok{\textquotesingle{}to\_torch\textquotesingle{}}\NormalTok{: }\KeywordTok{lambda} \VariableTok{self}\NormalTok{, data, gpu: torch.tensor(data, device}\OperatorTok{=}\NormalTok{gpu),}
\NormalTok{\})}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{5.14 Case Study 13: Bidirectional Type
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{class}\NormalTok{ BidirectionalTypeRegistry:}
    \KeywordTok{def} \FunctionTok{\_\_init\_\_}\NormalTok{(}\VariableTok{self}\NormalTok{):}
        \VariableTok{self}\NormalTok{.\_forward: Dict[Type, Type] }\OperatorTok{=}\NormalTok{ \{\}  }\CommentTok{\# lazy $\textbackslash{}rightarrow$ base}
        \VariableTok{self}\NormalTok{.\_reverse: Dict[Type, Type] }\OperatorTok{=}\NormalTok{ \{\}  }\CommentTok{\# base $\textbackslash{}rightarrow$ lazy}

    \KeywordTok{def}\NormalTok{ register(}\VariableTok{self}\NormalTok{, lazy\_type: Type, base\_type: Type):}
        \CommentTok{\# Single source of truth: type identity enforces bijection}
        \ControlFlowTok{if}\NormalTok{ lazy\_type }\KeywordTok{in} \VariableTok{self}\NormalTok{.\_forward:}
            \ControlFlowTok{raise} \PreprocessorTok{ValueError}\NormalTok{(}\SpecialStringTok{f"}\SpecialCharTok{\{}\NormalTok{lazy\_type}\SpecialCharTok{\}}\SpecialStringTok{ already registered"}\NormalTok{)}
        \ControlFlowTok{if}\NormalTok{ base\_type }\KeywordTok{in} \VariableTok{self}\NormalTok{.\_reverse:}
            \ControlFlowTok{raise} \PreprocessorTok{ValueError}\NormalTok{(}\SpecialStringTok{f"}\SpecialCharTok{\{}\NormalTok{base\_type}\SpecialCharTok{\}}\SpecialStringTok{ already has lazy companion"}\NormalTok{)}

        \VariableTok{self}\NormalTok{.\_forward[lazy\_type] }\OperatorTok{=}\NormalTok{ base\_type}
        \VariableTok{self}\NormalTok{.\_reverse[base\_type] }\OperatorTok{=}\NormalTok{ lazy\_type}

\CommentTok{\# Type identity as key ensures sync}
\NormalTok{registry.register(LazyPathPlanningConfig, PathPlanningConfig)}
\CommentTok{\# Later: registry.normalize(LazyPathPlanningConfig) $\textbackslash{}rightarrow$ PathPlanningConfig}
\CommentTok{\#        registry.get\_lazy(PathPlanningConfig) $\textbackslash{}rightarrow$ LazyPathPlanningConfig}
\end{Highlighting}
\end{Shaded}
...
\subsection{6. Formalization and
...
\item
  ...
  vs O(k) vs Ω(n) complexity separation. Proves that nominal error
  localization is O(1), structural is O(k), duck is Ω(n), and the gap
  grows without bound.
\end{enumerate}
...
\subsubsection{6.1 Type Universe and
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} Types are represented as natural numbers (nominal identity)}
\NormalTok{abbrev Typ := Nat}

\NormalTok{{-}{-} The lazy{-}to{-}base registry as a partial function}
\NormalTok{def Registry := Typ $\textbackslash{}rightarrow$ Option Typ}

\NormalTok{{-}{-} A registry is well{-}formed if base types are not in domain}
\NormalTok{def Registry.wellFormed (R : Registry) : Prop :=}
\NormalTok{  $\textbackslash{}forall$ L B, R L = some B $\textbackslash{}rightarrow$ R B = none}

\NormalTok{{-}{-} Normalization: map lazy type to base, or return unchanged}
\NormalTok{def normalizeType (R : Registry) (T : Typ) : Typ :=}
\NormalTok{  match R T with}
\NormalTok{  | some B =\textgreater{} B}
\NormalTok{  | none =\textgreater{} T}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{theorem normalizeType\_idempotent (R : Registry) (T : Typ)}
\NormalTok{    (h\_wf : R.wellFormed) :}
\NormalTok{    normalizeType R (normalizeType R T) = normalizeType R T := by}
\NormalTok{  simp only [normalizeType]}
\NormalTok{  cases hR : R T with}
\NormalTok{  | none =\textgreater{} simp only [hR]}
\NormalTok{  | some B =\textgreater{}}
\NormalTok{    have h\_base : R B = none := h\_wf T B hR}
\NormalTok{    simp only [h\_base]}
\end{Highlighting}
\end{Shaded}

\subsubsection{6.2 MRO and Scope Stack}\label{mro-and-scope-stack}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} MRO is a list of types, most specific first}
\NormalTok{abbrev MRO := List Typ}

\NormalTok{{-}{-} Scope stack: most specific first}
\NormalTok{abbrev ScopeStack := List ScopeId}

\NormalTok{{-}{-} Config instance: type and field value}
\NormalTok{structure ConfigInstance where}
\NormalTok{  typ : Typ}
\NormalTok{  fieldValue : FieldValue}

\NormalTok{{-}{-} Configs available at each scope}
\NormalTok{def ConfigContext := ScopeId $\textbackslash{}rightarrow$ List ConfigInstance}
\end{Highlighting}
\end{Shaded}

\subsubsection{6.3 The RESOLVE Algorithm}\label{the-resolve-algorithm}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} Resolution result: value, scope, source type}
\NormalTok{structure ResolveResult where}
\NormalTok{  value : FieldValue}
\NormalTok{  scope : ScopeId}
\NormalTok{  sourceType : Typ}
\NormalTok{deriving DecidableEq}

\NormalTok{{-}{-} Find first matching config in a list}
\NormalTok{def findConfigByType (configs : List ConfigInstance) (T : Typ) :}
\NormalTok{    Option FieldValue :=}
\NormalTok{  match configs.find? (fun c =\textgreater{} c.typ == T) with}
\NormalTok{  | some c =\textgreater{} some c.fieldValue}
\NormalTok{  | none =\textgreater{} none}

\NormalTok{{-}{-} The dual{-}axis resolution algorithm}
\NormalTok{def resolve (R : Registry) (mro : MRO)}
\NormalTok{    (scopes : ScopeStack) (ctx : ConfigContext) :}
\NormalTok{    Option ResolveResult :=}
\NormalTok{  {-}{-} X{-}axis: iterate scopes (most to least specific)}
\NormalTok{  scopes.findSome? fun scope =\textgreater{}}
\NormalTok{    {-}{-} Y{-}axis: iterate MRO (most to least specific)}
\NormalTok{    mro.findSome? fun mroType =\textgreater{}}
\NormalTok{      let normType := normalizeType R mroType}
\NormalTok{      match findConfigByType (ctx scope) normType with}
\NormalTok{      | some v =\textgreater{}}
\NormalTok{        if v $\textbackslash{}neq$ 0 then some ⟨v, scope, normType⟩}
\NormalTok{        else none}
\NormalTok{      | none =\textgreater{} none}
\end{Highlighting}
\end{Shaded}

\subsubsection{6.4 GETATTRIBUTE
Implementation}\label{getattribute-implementation}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} Raw field access (before resolution)}
\NormalTok{def rawFieldValue (obj : ConfigInstance) : FieldValue :=}
\NormalTok{  obj.fieldValue}

\NormalTok{{-}{-} GETATTRIBUTE implementation}
\NormalTok{def getattribute (R : Registry) (obj : ConfigInstance) (mro : MRO)}
\NormalTok{    (scopes : ScopeStack) (ctx : ConfigContext) (isLazyField : Bool) :}
\NormalTok{    FieldValue :=}
\NormalTok{  let raw := rawFieldValue obj}
\NormalTok{  if raw $\textbackslash{}neq$ 0 then raw  {-}{-} Concrete value, no resolution}
\NormalTok{  else if isLazyField then}
\NormalTok{    match resolve R mro scopes ctx with}
\NormalTok{    | some result =\textgreater{} result.value}
\NormalTok{    | none =\textgreater{} 0}
\NormalTok{  else raw}
\end{Highlighting}
\end{Shaded}

\subsubsection{6.5 Theorem 6.1: Resolution
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{theorem resolution\_completeness}
\NormalTok{    (R : Registry) (mro : MRO)}
\NormalTok{    (scopes : ScopeStack) (ctx : ConfigContext) (v : FieldValue) :}
\NormalTok{    (match resolve R mro scopes ctx with}
\NormalTok{     | some r =\textgreater{} r.value}
\NormalTok{     | none =\textgreater{} 0) = v $\textbackslash{}leftrightarrow$}
\NormalTok{    (v = 0 $\textbackslash{}land$ resolve R mro scopes ctx = none) $\textbackslash{}lor$}
\NormalTok{    ($\textbackslash{}exists$ r : ResolveResult,}
\NormalTok{      resolve R mro scopes ctx = some r $\textbackslash{}land$ r.value = v) := by}
\NormalTok{  cases hr : resolve R mro scopes ctx with}
\NormalTok{  | none =\textgreater{}}
\NormalTok{    constructor}
\NormalTok{    · intro h; left; exact ⟨h.symm, rfl⟩}
\NormalTok{    · intro h}
\NormalTok{      rcases h with ⟨hv, \_⟩ | ⟨r, hfalse, \_⟩}
\NormalTok{      · exact hv.symm}
\NormalTok{      · cases hfalse}
\NormalTok{  | some result =\textgreater{}}
\NormalTok{    constructor}
\NormalTok{    · intro h; right; exact ⟨result, rfl, h⟩}
\NormalTok{    · intro h}
\NormalTok{      rcases h with ⟨\_, hfalse⟩ | ⟨r, hr2, hv⟩}
\NormalTok{      · cases hfalse}
\NormalTok{      · simp only [Option.some.injEq] at hr2}
\NormalTok{        rw [$\textbackslash{}leftarrow$ hr2] at hv; exact hv}
\end{Highlighting}
\end{Shaded}

\subsubsection{6.6 Theorem 6.2: Provenance
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{theorem provenance\_uniqueness}
\NormalTok{    (R : Registry) (mro : MRO) (scopes : ScopeStack) (ctx : ConfigContext)}
\NormalTok{    (result\_1 result\_2 : ResolveResult)}
\NormalTok{    (hr\_1 : resolve R mro scopes ctx = some result\_1)}
\NormalTok{    (hr\_2 : resolve R mro scopes ctx = some result\_2) :}
\NormalTok{    result\_1 = result\_2 := by}
\NormalTok{  simp only [hr\_1, Option.some.injEq] at hr\_2}
\NormalTok{  exact hr\_2}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{theorem resolution\_determinism}
\NormalTok{    (R : Registry) (mro : MRO) (scopes : ScopeStack) (ctx : ConfigContext) :}
\NormalTok{    $\textbackslash{}forall$ r\_1 r\_2, resolve R mro scopes ctx = r\_1 $\textbackslash{}rightarrow$}
\NormalTok{             resolve R mro scopes ctx = r\_2 $\textbackslash{}rightarrow$}
\NormalTok{             r\_1 = r\_2 := by}
\NormalTok{  intros r\_1 r\_2 h\_1 h\_2}
\NormalTok{  rw [$\textbackslash{}leftarrow$ h\_1, $\textbackslash{}leftarrow$ h\_2]}
\end{Highlighting}
\end{Shaded}

\subsubsection{6.7 Duck Typing
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} In duck typing, a "type" is just a bag of (field\_name, field\_value) pairs}
\NormalTok{{-}{-} There\textquotesingle{}s no nominal identity {-} only structure matters}
\NormalTok{structure DuckObject where}
\NormalTok{  fields : List (String $\textbackslash{}times$ Nat)}
\NormalTok{deriving DecidableEq}

\NormalTok{{-}{-} Field lookup in a duck object}
\NormalTok{def getField (obj : DuckObject) (name : String) : Option Nat :=}
\NormalTok{  match obj.fields.find? (fun p =\textgreater{} p.1 == name) with}
\NormalTok{  | some p =\textgreater{} some p.2}
\NormalTok{  | none =\textgreater{} none}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} Two duck objects are "structurally equivalent" if they have same fields}
\NormalTok{{-}{-} This is THE defining property of duck typing: identity = structure}
\NormalTok{def structurallyEquivalent (a b : DuckObject) : Prop :=}
\NormalTok{  $\textbackslash{}forall$ name, getField a name = getField b name}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{theorem structEq\_refl (a : DuckObject) :}
\NormalTok{  structurallyEquivalent a a := by}
\NormalTok{  intro name; rfl}

\NormalTok{theorem structEq\_symm (a b : DuckObject) :}
\NormalTok{    structurallyEquivalent a b $\textbackslash{}rightarrow$ structurallyEquivalent b a := by}
\NormalTok{  intro h name; exact (h name).symm}

\NormalTok{theorem structEq\_trans (a b c : DuckObject) :}
\NormalTok{    structurallyEquivalent a b $\textbackslash{}rightarrow$ structurallyEquivalent b c $\textbackslash{}rightarrow$}
\NormalTok{    structurallyEquivalent a c := by}
\NormalTok{  intro hab hbc name; rw [hab name, hbc name]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} A duck{-}respecting function treats structurally equivalent objects identically}
\NormalTok{def DuckRespecting (f : DuckObject $\textbackslash{}rightarrow$ $\textbackslash{}alpha$) : Prop :=}
\NormalTok{  $\textbackslash{}forall$ a b, structurallyEquivalent a b $\textbackslash{}rightarrow$ f a = f b}
\end{Highlighting}
\end{Shaded}

\subsubsection{6.8 Corollary 6.3: Duck Typing Cannot Provide
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} Suppose we try to build a provenance function for duck typing}
\NormalTok{{-}{-} It would have to return which DuckObject provided the value}
\NormalTok{structure DuckProvenance where}
\NormalTok{  value : Nat}
\NormalTok{  source : DuckObject  {-}{-} "Which object provided this?"}
\NormalTok{deriving DecidableEq}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{theorem duck\_provenance\_indistinguishable}
\NormalTok{    (getProvenance : DuckObject $\textbackslash{}rightarrow$ Option DuckProvenance)}
\NormalTok{    (h\_duck : DuckRespecting getProvenance)}
\NormalTok{    (obj1 obj2 : DuckObject)}
\NormalTok{    (h\_equiv : structurallyEquivalent obj1 obj2) :}
\NormalTok{    getProvenance obj1 = getProvenance obj2 := by}
\NormalTok{  exact h\_duck obj1 obj2 h\_equiv}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{theorem duck\_provenance\_absurdity}
\NormalTok{    (getProvenance : DuckObject $\textbackslash{}rightarrow$ Option DuckProvenance)}
\NormalTok{    (h\_duck : DuckRespecting getProvenance)}
\NormalTok{    (obj1 obj2 : DuckObject)}
\NormalTok{    (h\_equiv : structurallyEquivalent obj1 obj2)}
\NormalTok{    (prov1 prov2 : DuckProvenance)}
\NormalTok{    (h1 : getProvenance obj1 = some prov1)}
\NormalTok{    (h2 : getProvenance obj2 = some prov2) :}
\NormalTok{    prov1 = prov2 := by}
\NormalTok{  have h\_eq := h\_duck obj1 obj2 h\_equiv}
\NormalTok{  rw [h1, h2] at h\_eq}
\NormalTok{  exact Option.some.inj h\_eq}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} Example: Two nominally different types}
\NormalTok{def WellFilterConfigType : Nat := 1}
\NormalTok{def StepWellFilterConfigType : Nat := 2}

\NormalTok{{-}{-} These are distinguishable despite potentially having same structure}
\NormalTok{theorem nominal\_types\_distinguishable :}
\NormalTok{    WellFilterConfigType $\textbackslash{}neq$ StepWellFilterConfigType := by decide}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{6.9 Verification Status}\label{verification-status}
...
\subsubsection{6.10 What the Lean Proofs
...
\subsubsection{6.11 External Provenance Map
...
\KeywordTok{class}\NormalTok{ B(A):}
    \ControlFlowTok{pass}  \CommentTok{\# Inherits x from A}

\NormalTok{b }\OperatorTok{=}\NormalTok{ B()}
\BuiltInTok{print}\NormalTok{(b.x)  }\CommentTok{\# Prints 1. Which type provided this?}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{6.12 Abstract Model Lean
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} The three axes of a class system}
\NormalTok{inductive Axis where}
\NormalTok{  | Name       {-}{-} N: type identifier}
\NormalTok{  | Bases      {-}{-} B: inheritance hierarchy}
\NormalTok{  | Namespace  {-}{-} S: attribute declarations (shape)}
\NormalTok{deriving DecidableEq, Repr}

\NormalTok{{-}{-} A typing discipline is characterized by which axes it inspects}
\NormalTok{abbrev AxisSet := List Axis}

\NormalTok{{-}{-} Canonical axis sets}
\NormalTok{def shapeAxes : AxisSet := [.Name, .Namespace]  {-}{-} Structural/duck typing}
\NormalTok{def nominalAxes : AxisSet := [.Name, .Bases, .Namespace]  {-}{-} Full nominal}

\NormalTok{{-}{-} Unified capability (combines typing and architecture domains)}
\NormalTok{inductive UnifiedCapability where}
\NormalTok{  | interfaceCheck      {-}{-} Check interface satisfaction}
\NormalTok{  | identity            {-}{-} Type identity}
\NormalTok{  | provenance          {-}{-} Type provenance}
\NormalTok{  | enumeration         {-}{-} Subtype enumeration}
\NormalTok{  | conflictResolution  {-}{-} MRO{-}based resolution}
\NormalTok{deriving DecidableEq, Repr}

\NormalTok{{-}{-} Capabilities enabled by each axis}
\NormalTok{def axisCapabilities (a : Axis) : List UnifiedCapability :=}
\NormalTok{  match a with}
\NormalTok{  | .Name =\textgreater{} [.interfaceCheck]}
\NormalTok{  | .Bases =\textgreater{} [.identity, .provenance, .enumeration, .conflictResolution]}
\NormalTok{  | .Namespace =\textgreater{} [.interfaceCheck]}

\NormalTok{{-}{-} Capabilities of an axis set = union of each axis\textquotesingle{}s capabilities}
\NormalTok{def axisSetCapabilities (axes : AxisSet) : List UnifiedCapability :=}
\NormalTok{  axes.flatMap axisCapabilities |\textgreater{}.eraseDups}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} THEOREM: Shape axes $\textbackslash{}subset$ Nominal axes (specific instance of lattice ordering)}
\NormalTok{theorem axis\_shape\_subset\_nominal :}
\NormalTok{    $\textbackslash{}forall$ c $\textbackslash{}in$ axisSetCapabilities shapeAxes,}
\NormalTok{      c $\textbackslash{}in$ axisSetCapabilities nominalAxes := by}
\NormalTok{  intro c hc}
\NormalTok{  have h\_shape : axisSetCapabilities shapeAxes = [UnifiedCapability.interfaceCheck] := rfl}
\NormalTok{  have h\_nominal : UnifiedCapability.interfaceCheck $\textbackslash{}in$ axisSetCapabilities nominalAxes := by decide}
\NormalTok{  rw [h\_shape] at hc}
\NormalTok{  simp only [List.mem\_singleton] at hc}
\NormalTok{  rw [hc]}
\NormalTok{  exact h\_nominal}

\NormalTok{{-}{-} THEOREM: Nominal has capabilities Shape lacks}
\NormalTok{theorem axis\_nominal\_exceeds\_shape :}
\NormalTok{    $\textbackslash{}exists$ c $\textbackslash{}in$ axisSetCapabilities nominalAxes,}
\NormalTok{      c $\textbackslash{}notin$ axisSetCapabilities shapeAxes := by}
\NormalTok{  use UnifiedCapability.provenance}
\NormalTok{  constructor}
\NormalTok{  · decide  {-}{-} provenance $\textbackslash{}in$ nominalAxes capabilities}
\NormalTok{  · decide  {-}{-} provenance $\textbackslash{}notin$ shapeAxes capabilities}

\NormalTok{{-}{-} THE LATTICE METATHEOREM: Combined strict dominance}
\NormalTok{theorem lattice\_dominance :}
\NormalTok{    ($\textbackslash{}forall$ c $\textbackslash{}in$ axisSetCapabilities shapeAxes, c $\textbackslash{}in$ axisSetCapabilities nominalAxes) $\textbackslash{}land$}
\NormalTok{    ($\textbackslash{}exists$ c $\textbackslash{}in$ axisSetCapabilities nominalAxes, c $\textbackslash{}notin$ axisSetCapabilities shapeAxes) :=}
\NormalTok{  ⟨axis\_shape\_subset\_nominal, axis\_nominal\_exceeds\_shape⟩}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} All possible capabilities in the system}
\NormalTok{inductive Capability where}
\NormalTok{  | interfaceCheck      {-}{-} "Does x have method m?"}
\NormalTok{  | typeNaming          {-}{-} "What is the name of type T?"}
\NormalTok{  | valueAccess         {-}{-} "What is x.a?"}
\NormalTok{  | methodInvocation    {-}{-} "Call x.m()"}
\NormalTok{  | provenance          {-}{-} "Which type provided this value?"}
\NormalTok{  | identity            {-}{-} "Is x an instance of T?"}
\NormalTok{  | enumeration         {-}{-} "What are all subtypes of T?"}
\NormalTok{  | conflictResolution  {-}{-} "Which definition wins in diamond?"}
\NormalTok{deriving DecidableEq, Repr}

\NormalTok{{-}{-} Capabilities that require the Bases axis}
\NormalTok{def basesRequiredCapabilities : List Capability :=}
\NormalTok{  [.provenance, .identity, .enumeration, .conflictResolution]}

\NormalTok{{-}{-} Capabilities that do NOT require Bases (only need N or S)}
\NormalTok{def nonBasesCapabilities : List Capability :=}
\NormalTok{  [.interfaceCheck, .typeNaming, .valueAccess, .methodInvocation]}

\NormalTok{{-}{-} THEOREM: Bases capabilities are exactly \{provenance, identity, enumeration, conflictResolution\}}
\NormalTok{theorem bases\_capabilities\_complete :}
\NormalTok{    ∀ c : Capability,}
\NormalTok{      (c ∈ basesRequiredCapabilities ↔}
\NormalTok{       c = .provenance ∨ c = .identity ∨ c = .enumeration ∨ c = .conflictResolution) := by}
\NormalTok{  intro c}
\NormalTok{  constructor}
\NormalTok{  · intro h}
\NormalTok{    simp [basesRequiredCapabilities] at h}
\NormalTok{    exact h}
\NormalTok{  · intro h}
\NormalTok{    simp [basesRequiredCapabilities]}
\NormalTok{    exact h}

\NormalTok{{-}{-} THEOREM: Non{-}Bases capabilities are exactly \{interfaceCheck, typeNaming, valueAccess, methodInvocation\}}
\NormalTok{theorem non\_bases\_capabilities\_complete :}
\NormalTok{    ∀ c : Capability,}
\NormalTok{      (c ∈ nonBasesCapabilities ↔}
\NormalTok{       c = .interfaceCheck ∨ c = .typeNaming ∨ c = .valueAccess ∨ c = .methodInvocation) := by}
\NormalTok{  intro c}
\NormalTok{  constructor}
\NormalTok{  · intro h}
\NormalTok{    simp [nonBasesCapabilities] at h}
\NormalTok{    exact h}
\NormalTok{  · intro h}
\NormalTok{    simp [nonBasesCapabilities]}
\NormalTok{    exact h}

\NormalTok{{-}{-} THEOREM: Every capability is in exactly one category (partition)}
\NormalTok{theorem capability\_partition :}
\NormalTok{    ∀ c : Capability,}
\NormalTok{      (c ∈ basesRequiredCapabilities ∨ c ∈ nonBasesCapabilities) ∧}
\NormalTok{      ¬(c ∈ basesRequiredCapabilities ∧ c ∈ nonBasesCapabilities) := by}
\NormalTok{  intro c}
\NormalTok{  cases c \textless{};\textgreater{} simp [basesRequiredCapabilities, nonBasesCapabilities]}

\NormalTok{{-}{-} THEOREM: |basesRequiredCapabilities| = 4 (exactly four capabilities)}
\NormalTok{theorem bases\_capabilities\_count :}
\NormalTok{    basesRequiredCapabilities.length = 4 := by rfl}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{6.13 Complexity Bounds
Formalization}\label{complexity-bounds-formalization}

We formalize the O(1) vs O(k) vs Ω(n) complexity claims from Section
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} A program has classes and call sites}
\NormalTok{structure Program where}
\NormalTok{  classes : List Nat      {-}{-} Class IDs}
\NormalTok{  callSites : List Nat    {-}{-} Call site IDs}
\NormalTok{  {-}{-} Which call sites use which attribute}
\NormalTok{  callSiteAttribute : Nat → String}
\NormalTok{  {-}{-} Which class declares a constraint}
\NormalTok{  constraintClass : String → Nat}

\NormalTok{{-}{-} A constraint is a requirement on an attribute}
\NormalTok{structure Constraint where}
\NormalTok{  attribute : String}
\NormalTok{  declaringSite : Nat  {-}{-} The class that declares the constraint}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{inductive CheckLocation where}
\NormalTok{  | classDefinition : Nat → CheckLocation  {-}{-} Checked at class definition}
\NormalTok{  | callSite : Nat → CheckLocation         {-}{-} Checked at call site}
\NormalTok{deriving DecidableEq}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} Nominal: check at the single class definition point}
\NormalTok{def nominalCheckLocations (p : Program) (c : Constraint) : List CheckLocation :=}
\NormalTok{  [.classDefinition c.declaringSite]}

\NormalTok{{-}{-} Structural: check at each implementing class (we model k implementing classes)}
\NormalTok{def structuralCheckLocations (p : Program) (c : Constraint)}
\NormalTok{    (implementingClasses : List Nat) : List CheckLocation :=}
\NormalTok{  implementingClasses.map CheckLocation.classDefinition}

\NormalTok{{-}{-} Duck: check at each call site that uses the attribute}
\NormalTok{def duckCheckLocations (p : Program) (c : Constraint) : List CheckLocation :=}
\NormalTok{  p.callSites.filter (fun cs =\textgreater{} p.callSiteAttribute cs == c.attribute)}
\NormalTok{             |\textgreater{}.map CheckLocation.callSite}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{theorem nominal\_check\_count\_is\_1 (p : Program) (c : Constraint) :}
\NormalTok{    (nominalCheckLocations p c).length = 1 := by}
\NormalTok{  simp [nominalCheckLocations]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{theorem structural\_check\_count\_is\_k (p : Program) (c : Constraint)}
\NormalTok{    (implementingClasses : List Nat) :}
\NormalTok{    (structuralCheckLocations p c implementingClasses).length =}
\NormalTok{    implementingClasses.length := by}
\NormalTok{  simp [structuralCheckLocations]}
\end{Highlighting}
\end{Shaded}

\textbf{Theorem 6.7 (Duck Ω(n)).} Duck typing checks n locations (n =
relevant call sites):

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} Helper: count call sites using an attribute}
\NormalTok{def relevantCallSites (p : Program) (attr : String) : List Nat :=}
\NormalTok{  p.callSites.filter (fun cs =\textgreater{} p.callSiteAttribute cs == attr)}

\NormalTok{theorem duck\_check\_count\_is\_n (p : Program) (c : Constraint) :}
\NormalTok{    (duckCheckLocations p c).length =}
\NormalTok{    (relevantCallSites p c.attribute).length := by}
\NormalTok{  simp [duckCheckLocations, relevantCallSites]}
\end{Highlighting}
\end{Shaded}

\textbf{Theorem 6.8 (Strict Ordering).} For non-trivial programs (k ≥ 1,
n ≥ k), the complexity ordering is strict:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} 1 ≤ k: Nominal dominates structural when there\textquotesingle{}s at least one implementing class}
\NormalTok{theorem nominal\_leq\_structural (p : Program) (c : Constraint)}
\NormalTok{    (implementingClasses : List Nat) (h : implementingClasses ≠ []) :}
\NormalTok{    (nominalCheckLocations p c).length ≤}
\NormalTok{    (structuralCheckLocations p c implementingClasses).length := by}
\NormalTok{  simp [nominalCheckLocations, structuralCheckLocations]}
\NormalTok{  exact Nat.one\_le\_iff\_ne\_zero.mpr (List.length\_pos\_of\_ne\_nil h |\textgreater{} Nat.not\_eq\_zero\_of\_lt)}

\NormalTok{{-}{-} k ≤ n: Structural dominates duck when call sites outnumber implementing classes}
\NormalTok{theorem structural\_leq\_duck (p : Program) (c : Constraint)}
\NormalTok{    (implementingClasses : List Nat)}
\NormalTok{    (h : implementingClasses.length ≤ (relevantCallSites p c.attribute).length) :}
\NormalTok{    (structuralCheckLocations p c implementingClasses).length ≤}
\NormalTok{    (duckCheckLocations p c).length := by}
\NormalTok{  simp [structuralCheckLocations, duckCheckLocations, relevantCallSites]}
\NormalTok{  exact h}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} Duck complexity can be arbitrarily large}
\NormalTok{theorem duck\_complexity\_unbounded :}
\NormalTok{    ∀ n : Nat, ∃ p c, (duckCheckLocations p c).length ≥ n := by}
\NormalTok{  intro n}
\NormalTok{  {-}{-} Construct program with n call sites all using attribute "foo"}
\NormalTok{  let p : Program := \{}
\NormalTok{    classes := [0],}
\NormalTok{    callSites := List.range n,}
\NormalTok{    callSiteAttribute := fun \_ =\textgreater{} "foo",}
\NormalTok{    constraintClass := fun \_ =\textgreater{} 0}
\NormalTok{  \}}
\NormalTok{  let c : Constraint := \{ attribute := "foo", declaringSite := 0 \}}
\NormalTok{  use p, c}
\NormalTok{  simp [duckCheckLocations, relevantCallSites, p, c]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} The gap: duck requires n checks where nominal requires 1}
\NormalTok{theorem error\_localization\_gap (p : Program) (c : Constraint)}
\NormalTok{    (h : (relevantCallSites p c.attribute).length = n) (hn : n ≥ 1) :}
\NormalTok{    (duckCheckLocations p c).length {-} (nominalCheckLocations p c).length = n {-} 1 := by}
\NormalTok{  simp [duckCheckLocations, nominalCheckLocations, relevantCallSites] at *}
\NormalTok{  omega}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{6.14 The Unarguable Theorems (Lean
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} THEOREM 3.13: Provenance is not shape{-}respecting when distinct types share namespace}
\NormalTok{{-}{-} Therefore no shape discipline can compute provenance}
\NormalTok{theorem provenance\_not\_shape\_respecting (ns : Namespace) (bases : Bases)}
\NormalTok{    {-}{-} Premise: there exist two types with same namespace but different bases}
\NormalTok{    (A B : Typ)}
\NormalTok{    (h\_same\_ns : shapeEquivalent ns A B)}
\NormalTok{    (h\_diff\_bases : bases A ≠ bases B)}
\NormalTok{    {-}{-} Any provenance function that distinguishes them}
\NormalTok{    (prov : ProvenanceFunction)}
\NormalTok{    (h\_distinguishes : prov A "x" ≠ prov B "x") :}
\NormalTok{    {-}{-} Cannot be computed by a shape discipline}
\NormalTok{    ¬ShapeRespecting ns (fun T =\textgreater{} prov T "x") := by}
\NormalTok{  intro h\_shape\_resp}
\NormalTok{  {-}{-} If prov were shape{-}respecting, then prov A "x" = prov B "x"}
\NormalTok{  have h\_eq : prov A "x" = prov B "x" := h\_shape\_resp A B h\_same\_ns}
\NormalTok{  {-}{-} But we assumed prov A "x" ≠ prov B "x"}
\NormalTok{  exact h\_distinguishes h\_eq}

\NormalTok{{-}{-} COROLLARY: Provenance impossibility is universal}
\NormalTok{theorem provenance\_impossibility\_universal :}
\NormalTok{    ∀ (ns : Namespace) (A B : Typ),}
\NormalTok{      shapeEquivalent ns A B →}
\NormalTok{      ∀ (prov : ProvenanceFunction),}
\NormalTok{        prov A "x" ≠ prov B "x" →}
\NormalTok{        ¬ShapeRespecting ns (fun T =\textgreater{} prov T "x") := by}
\NormalTok{  intro ns A B h\_eq prov h\_neq h\_shape}
\NormalTok{  exact h\_neq (h\_shape A B h\_eq)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} Query space partitions EXACTLY into shape{-}respecting and B{-}dependent}
\NormalTok{{-}{-} This is Theorem 3.18 (Query Space Partition)}
\NormalTok{theorem query\_space\_partition (ns : Namespace) (q : SingleQuery) :}
\NormalTok{    (ShapeRespectingSingle ns q ∨ BasesDependentQuery ns q) ∧}
\NormalTok{    ¬(ShapeRespectingSingle ns q ∧ BasesDependentQuery ns q) := by}
\NormalTok{  constructor}
\NormalTok{  · {-}{-} Exhaustiveness: either shape{-}respecting or bases{-}dependent}
\NormalTok{    by\_cases h : ShapeRespectingSingle ns q}
\NormalTok{    · left; exact h}
\NormalTok{    · right}
\NormalTok{      simp only [ShapeRespectingSingle, not\_forall] at h}
\NormalTok{      obtain ⟨A, B, h\_eq, h\_neq⟩ := h}
\NormalTok{      exact ⟨A, B, h\_eq, h\_neq⟩}
\NormalTok{  · {-}{-} Mutual exclusion: cannot be both}
\NormalTok{    intro ⟨h\_shape, h\_bases⟩}
\NormalTok{    obtain ⟨A, B, h\_eq, h\_neq⟩ := h\_bases}
\NormalTok{    have h\_same : q A = q B := h\_shape A B h\_eq}
\NormalTok{    exact h\_neq h\_same}
\end{Highlighting}
\end{Shaded}
...
requires Ω(n) inspections:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-} THEOREM: In the worst case, finding the error source requires n{-}1 inspections}
\NormalTok{theorem error\_localization\_lower\_bound (n : Nat) (hn : n ≥ 1) :}
\NormalTok{    {-}{-} For any sequence of n{-}2 or fewer inspections...}
\NormalTok{    ∀ (inspections : List (Fin n)),}
\NormalTok{      inspections.length \textless{} n {-} 1 →}
\NormalTok{      {-}{-} There exist two different error configurations}
\NormalTok{      {-}{-} that are consistent with all inspection results}
\NormalTok{      ∃ (src1 src2 : Fin n),}
\NormalTok{        src1 ≠ src2 ∧}
\NormalTok{        src1 ∉ inspections ∧ src2 ∉ inspections := by}
\NormalTok{  intro inspections h\_len}
\NormalTok{  {-}{-} Counting argument: if |inspections| \textless{} n{-}1, then |uninspected| ≥ 2}
\NormalTok{  have h\_uninspected : n {-} inspections.length ≥ 2 := by omega}
\NormalTok{  {-}{-} Therefore at least 2 uninspected sites exist (adversary\textquotesingle{}s freedom)}
\NormalTok{  {-}{-} Pigeonhole counting argument (fully formalized in actual Lean file)}

\NormalTok{{-}{-} COROLLARY: The complexity gap is unbounded}
\NormalTok{theorem complexity\_gap\_unbounded :}
\NormalTok{    ∀ (k : Nat), ∃ (n : Nat), n {-} 1 \textgreater{} k := by}
\NormalTok{  intro k}
\NormalTok{  use k + 2}
\NormalTok{  omega}
\end{Highlighting}
...
algorithm can be forced to make Ω(n) inspections---the adversary answers
...
\subsubsection{7.2 Practical Hybrid
...
\subsubsection{7.3 Metaprogramming
...
\subsubsection{7.4 Behavioral Subtyping}\label{behavioral-subtyping}
...
\subsubsection{7.5 Positioning This Work}\label{positioning-this-work}
...
preprints with ≥10 citations; addresses nominal vs structural typing
...
absence of any work scoring ≥1/5 in Table 7.5.3 is not a gap in our
...
scoring ≥1/5 in Table 7.5.3. The 0/5 scores across all surveyed work are
...
\subsubsection{8.2 The Typing Discipline
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Structural approach (Protocol) {-} implicit}
\AttributeTok{@runtime\_checkable}
\KeywordTok{class}\NormalTok{ Configurable(Protocol):}
    \KeywordTok{def}\NormalTok{ validate(}\VariableTok{self}\NormalTok{) }\OperatorTok{{-}\textgreater{}} \BuiltInTok{bool}\NormalTok{: ...}

\BuiltInTok{isinstance}\NormalTok{(their\_obj, Configurable)  }\CommentTok{\# Hope methods match}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Nominal approach (Adapter) {-} explicit}
\KeywordTok{class}\NormalTok{ TheirTypeAdapter(TheirType, ConfigurableABC):}
    \ControlFlowTok{pass}  \CommentTok{\# 2 lines. Now in your hierarchy.}

\NormalTok{adapted }\OperatorTok{=}\NormalTok{ TheirTypeAdapter(their\_obj)  }\CommentTok{\# Explicit boundary}
\BuiltInTok{isinstance}\NormalTok{(adapted, ConfigurableABC)   }\CommentTok{\# Nominal check}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{8.4 Implications for Language
...
\subsubsection{8.5 Derivable Code Quality
...
\subsubsection{8.6 Hybrid Systems and Methodology
...
\(B = \emptyset\)
Criterion}{8.6.2 The B \textbackslash neq \textbackslash emptyset vs B = \textbackslash emptyset Criterion}}\label{the-b-neq-emptyset-vs-b-emptyset-criterion}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# B ≠ ∅: internal config hierarchy (use nominal)}
\KeywordTok{class}\NormalTok{ ConfigBase(ABC):}
    \AttributeTok{@abstractmethod}
    \KeywordTok{def}\NormalTok{ validate(}\VariableTok{self}\NormalTok{) }\OperatorTok{{-}\textgreater{}} \BuiltInTok{bool}\NormalTok{: }\ControlFlowTok{pass}

\KeywordTok{class}\NormalTok{ PathPlanningConfig(ConfigBase):}
\NormalTok{    well\_filter: Optional[}\BuiltInTok{str}\NormalTok{]}

\CommentTok{\# B = ∅: parse external JSON (structural is only option)}
\KeywordTok{def}\NormalTok{ load\_config\_from\_json(json\_dict: Dict[}\BuiltInTok{str}\NormalTok{, Any]) }\OperatorTok{{-}\textgreater{}}\NormalTok{ ConfigBase:}
    \CommentTok{\# JSON has no inheritance—structural validation at boundary}
    \ControlFlowTok{if} \StringTok{"well\_filter"} \KeywordTok{in}\NormalTok{ json\_dict:}
        \ControlFlowTok{return}\NormalTok{ PathPlanningConfig(}\OperatorTok{**}\NormalTok{json\_dict)  }\CommentTok{\# Returns nominal type}
    \ControlFlowTok{raise} \PreprocessorTok{ValueError}\NormalTok{(}\StringTok{"Invalid config"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\textbf{Removed rows:} - ``Retrofit / external types → Structural
(acceptable)'' --- \textbf{Wrong.} Adapters exist. Theorem 2.10j. -
``Small scripts / prototypes → Duck (acceptable)'' --- \textbf{Wrong.}
...
\subsubsection{8.7 Case Study: TypeScript's Design
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{class}\NormalTok{ A \{ x}\OperatorTok{:} \DataTypeTok{number} \OperatorTok{=} \DecValTok{1}\OperatorTok{;}\NormalTok{ \}}
\KeywordTok{class}\NormalTok{ B \{ x}\OperatorTok{:} \DataTypeTok{number} \OperatorTok{=} \DecValTok{1}\OperatorTok{;}\NormalTok{ \}}

\CommentTok{// Runtime: instanceof creates distinction}
\KeywordTok{const}\NormalTok{ b }\OperatorTok{=} \KeywordTok{new} \FunctionTok{B}\NormalTok{()}\OperatorTok{;}
\BuiltInTok{console}\OperatorTok{.}\FunctionTok{log}\NormalTok{(b }\KeywordTok{instanceof}\NormalTok{ A)}\OperatorTok{;}  \CommentTok{// false {-} different classes}

\CommentTok{// Type system: no distinction}
\KeywordTok{function} \FunctionTok{f}\NormalTok{(a}\OperatorTok{:}\NormalTok{ A) \{ \}}
\FunctionTok{f}\NormalTok{(}\KeywordTok{new} \FunctionTok{B}\NormalTok{())}\OperatorTok{;}  \CommentTok{// OK {-} same structure}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{// Workaround: add a private field to force nominal distinction}
\KeywordTok{class}\NormalTok{ StepWellFilterConfig }\KeywordTok{extends}\NormalTok{ WellFilterConfig \{}
    \KeywordTok{private}\NormalTok{ \_\_brand}\OperatorTok{!:} \DataTypeTok{void}\OperatorTok{;}  \CommentTok{// Forces nominal identity}
\NormalTok{\}}

\CommentTok{// Now TypeScript treats them as distinct (private field differs)}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{8.8 Mixins with MRO Strictly Dominate Object
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Mixin: behavior provider via inheritance}
\KeywordTok{class}\NormalTok{ LoggingMixin:}
    \KeywordTok{def}\NormalTok{ process(}\VariableTok{self}\NormalTok{):}
        \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"Logging: }\SpecialCharTok{\{}\VariableTok{self}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}
        \BuiltInTok{super}\NormalTok{().process()}

\KeywordTok{class}\NormalTok{ CachingMixin:}
    \KeywordTok{def}\NormalTok{ process(}\VariableTok{self}\NormalTok{):}
        \ControlFlowTok{if}\NormalTok{ cached }\OperatorTok{:=} \VariableTok{self}\NormalTok{.\_check\_cache():}
            \ControlFlowTok{return}\NormalTok{ cached}
\NormalTok{        result }\OperatorTok{=} \BuiltInTok{super}\NormalTok{().process()}
        \VariableTok{self}\NormalTok{.\_cache(result)}
        \ControlFlowTok{return}\NormalTok{ result}

\CommentTok{\# Composition via bases (single decision point)}
\KeywordTok{class}\NormalTok{ Handler(LoggingMixin, CachingMixin, BaseHandler):}
    \ControlFlowTok{pass}  \CommentTok{\# MRO: Handler $\textbackslash{}rightarrow$ Logging $\textbackslash{}rightarrow$ Caching $\textbackslash{}rightarrow$ Base}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Composition: behavior provider via delegation}
\KeywordTok{class}\NormalTok{ Handler:}
    \KeywordTok{def} \FunctionTok{\_\_init\_\_}\NormalTok{(}\VariableTok{self}\NormalTok{):}
        \VariableTok{self}\NormalTok{.logger }\OperatorTok{=}\NormalTok{ Logger()}
        \VariableTok{self}\NormalTok{.cache }\OperatorTok{=}\NormalTok{ Cache()}

    \KeywordTok{def}\NormalTok{ process(}\VariableTok{self}\NormalTok{):}
        \VariableTok{self}\NormalTok{.logger.log(}\VariableTok{self}\NormalTok{)  }\CommentTok{\# Manual dispatch point 1}
        \ControlFlowTok{if}\NormalTok{ cached }\OperatorTok{:=} \VariableTok{self}\NormalTok{.cache.check():  }\CommentTok{\# Manual dispatch point 2}
            \ControlFlowTok{return}\NormalTok{ cached}
\NormalTok{        result }\OperatorTok{=} \VariableTok{self}\NormalTok{.\_do\_process()}
        \VariableTok{self}\NormalTok{.cache.store(key, result)  }\CommentTok{\# Manual dispatch point 3}
        \ControlFlowTok{return}\NormalTok{ result}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{8.9 Validation: Alignment with Python's Design
...
\subsubsection{8.10 Connection to Gradual
...
\subsubsection{9.2 Application: LLM Code
...
(\(B \neq \emptyset\)) → isinstance/ABC patterns are correct; hasattr
patterns are violations (by Theorem 3.5) 2. If the prompt involves pure
data without inheritance (\(B = \emptyset\), e.g., JSON) → structural
patterns are the only option 3. External types requiring integration →
